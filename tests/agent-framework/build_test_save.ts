...
import { calcResources } from '../../src/sillytavern/tier-constants.js';
...
export function extractNarrative(aiMes: string, anchor: string): string {
  ...
  for (const tag of _stripTags) {
    // 删正文段内成对面板 + 孤立标签 (配对在段内完整, 非贪婪不会跨吞)
...
const playerRes = calcResources(3, player.attributes);
player.maxHp = playerRes.maxHp;
player.hp = playerRes.hp;
player.maxMp = playerRes.maxMp;
player.mp = playerRes.mp;
player.maxSp = playerRes.maxSp;
player.sp = playerRes.sp;
player.money = 2500;
player.location = '中南部-瓦伦蒂亚公国-诺瓦·瓦伦蒂亚城-中城区-私人住宅-仓库';
player.adventurerRank = 'B';
player.currentAction = '在仓库中发现石棺中的少女';
// M2: `CharacterState.equipment[]` 已删除 —— 装备不是独立实体，是 inventory 里
// `equippedSlot` 非空的物品（规范 §3）。这份夹具跟着现役形状走，别再复活 equipment[]。
player.inventory = [
  {
    name: '制式次元腰包',
    quantity: 1,
  ...
  },
  {
    ...
    equippedSlot: '身体',
    type: '装备',
    description: '绣有星辰图案的深蓝色华美法袍',
    stats: { 防御力: 45, 智力: 4 },
    durability: 120,
    maxDurability: 120,
    effects: {
      奥术亲和: '提升奥术法术伤害20%',
      魔力护盾: '受到攻击时自动展开魔力护盾吸收100点伤害',
    },
    scripts: {
      init: '$event.on("combat_action", "onOwnerHit", function(ctx) { $resource.modifyHp(ctx.owner, Math.min(100, ctx.event.damage || 0)); });',
    },
  },
  // 源夹具把这颗法球和法师之帽都放在「头部」——同槽双占，M2 的 equippedSlot 模型下不成立。
  // 按物品语义落到「饰品」（法球是手持/佩挂物，不是头盔）。
  {
    name: '聚能魔晶法球',
    quantity: 1,
    equippedSlot: '饰品',
    type: '装备',
    description: '一颗手掌大的透明水晶球，内部交织着各种元素的能量螺旋',
    stats: { 攻击力: 85, 智力: 5 },
    durability: 100,
    maxDurability: 100,
...
const qllRes = calcResources(5, qll.attributes);
qll.maxHp = qllRes.maxHp;
qll.hp = qllRes.hp;
qll.maxMp = qllRes.maxMp;
qll.mp = qllRes.mp;
qll.maxSp = qllRes.maxSp;
qll.sp = qllRes.sp;
qll.money = 0;
...
qll.customFields = {
  saveId: SAVE_ID,
  background:
    '绮萝莉娅曾是无尽树海深处某古代教团的圣女，数百年前在一次登神仪式中失败，灵魂与肉体分离，被封印于石棺中沉睡至今。',
  appearance:
    '冰蓝色及腰长发如冻结的瀑布，蓝粉混合色的双瞳透出非人的空灵。身材匀称但腹部异常隆起（仪式后遗症）。',
...
qll.ascension = {
  ...
  elements: [
    {
      name: '共鸣',
      description: '精神伤害攻击时10%几率与目标灵魂产生共鸣，无视20%精神抗性',
      effects: ['精神伤害共鸣', '抗性穿透20%'],
    },
  ],
  authority: [],
  law: [],
...
// M2: 已装备物品与背包物品同住 inventory，靠 equippedSlot 区分（规范 §3）
const qllEquipped: InventoryItem[] = [
  {
    name: '彼岸花法杖',
    quantity: 1,
    equippedSlot: '武器',
    type: '装备',
    description: '猩红水晶与未知枯木制成的法杖，顶端盛开永不凋零的彼岸花',
    stats: { 攻击力: 520, 精神: 8 },
    durability: 999,
    maxDurability: 999,
    ...
    scripts: {
      onRetire:
        '$status.add(nextAlly, { name: "凋零旋律", category: "增益", stacks: 1, duration: 15, timeUnit: "秒", effects: { "精神伤害": 25 } });',
    },
  },
  {
    name: '圣仪华裙',
    ...
    equippedSlot: '身体',
    type: '装备',
    description: '无尽树海教团圣女的仪式礼服，嵌有自动修复法阵',
    stats: { 防御力: 120, 精神: 4 },
    durability: 999,
    maxDurability: 999,
...
qll.skills = [
  {
    ...
    name: '序曲·止息',
    description:
      '挥动法杖发动至多四段连续攻击，造成精神伤害。命中敌人获得【乐章】灵感。可蓄力发动重击造成范围精神伤害。',
    type: 'active',
    level: 1,
    effects: { 连击: '至多四段攻击', 乐章积累: '每次命中获得【乐章】灵感' },
    scripts: {
      onHit:
        '$status.add(owner, { name: "乐章", category: "增益", stacks: 1, maxStacks: 100, stackable: true, effects: {} });',
    },
  },
  {
    id: 'sk-qll-02',
    ...
    scripts: {
      onCast:
        '$status.add(target, { name: "失调", category: "减益", stacks: 1, duration: 8, timeUnit: "秒", effects: { "精神抗性": -10 } });',
    },
  },
  {
    id: 'sk-qll-03',
    name: '终曲·赫卡忒',
    description:
      '消耗所有【协奏能量】，召唤猩红魔女人偶·赫卡忒登场协同攻击，造成大范围精神伤害。赫卡忒留在场上20秒，属性与召唤者一致。',
    type: 'active',
    cost: { type: 'MP', amount: 1200 },
    cooldown: 0,
    maxCooldown: 20,
    level: 1,
    ...
    scripts: {
      onCast:
        '$status.add(owner, { name: "赫卡忒召唤", category: "增益", stacks: 1, duration: 20, timeUnit: "秒", effects: {} });',
      cleanup: '$status.remove(owner, "赫卡忒召唤");',
    },
  },
  {
    id: 'sk-qll-04',
    name: '华彩乐段',
    description:
      '战斗中积攒【乐章】灵感(上限100点)。灵感满溢时蓄力重击升华为【华彩·猩红回响】，转化所有灵感为【协奏能量】。',
    type: 'passive',
    level: 1,
    effects: { 乐章上限: '100点', 转化: '满溢时转为【协奏能量】' },
    scripts: {
      init: '$event.on("combat_action", "onFullStacks", function(ctx) { var s = $status.getStacks(ctx.owner, "乐章"); if (s >= 100) { $status.setStacks(ctx.owner, "乐章", 0); $status.add(ctx.owner, { name: "协奏能量", category: "增益", stacks: 1, maxStacks: 1, stackable: false, effects: {} }); } });',
    },
  },
  {
    id: 'sk-qll-05',
    name: '声骸共生',
    description:
      '受到致命伤害时免疫此次伤害，强制赫卡忒代为承受并毁灭。触发后进入70秒【寂静】状态无法施放。',
    type: 'passive',
    level: 1,
...
qll.statusEffects = [
  {
    id: 'se-soulsplit',
    name: '灵魂分离',
    description: '灵魂无法操控身体，只能以灵体对话。身体对外界指令有最低限度的自主反应。',
    category: '特殊',
    stacks: 1,
    maxStacks: 1,
    stackable: false,
    ...
    effects: {},
    effectDescriptions: {
      灵体对话: '角色可以使用灵体进行交流，但无法通过身体施放技能或进行物理攻击',
      自主反应: '身体对语言指令有最低限度的反应，可能自发执行简单动作',
    },
  },
];
qll.inventory = [
  ...qllEquipped,
  {
    id: 'inv-qll-badge',
    name: '远古教团圣徽',
    description: '无尽树海古代教团的圣徽，其上镌刻着失传的仪式符文。可能在教团遗址触发隐藏事件。',
    quantity: 1,
    type: '任务物品',
    rarity: '传说' as const,
  ...
  },
  {
    id: 'inv-qll-soul',
    name: '魂晶碎片',
    description: '封印灵魂能量的结晶体碎片，可用于临时强化灵体类技能或作为高级炼金材料。',
    quantity: 3,
    type: '消耗品',
    rarity: '稀有' as const,
...
const baseVars: Record<string, any> = {
  ...
  任务列表: {
    '深绿之心 (团队)': { 状态: '已结算', 详情: '受瓦伦蒂亚公国委托，深入无尽树海寻找古代遗物' },
  },
  命运点数: 200,
};
...
const memTemplates = [
  null as any,
  {
    c: '柠萌茶将A级任务奖励古代石棺擦拭后，发现棺盖接缝。掀开石棺后发现里面沉睡着名叫绮萝莉娅的少女。少女拥有冰蓝色长发和蓝粉混合色异瞳，身着华丽白裙，腹部高高隆起却仍是处女。她以灵体对话与柠萌茶交流，自称是古老教团的圣女，数百年前在一次登神仪式中失败后被封印于此。',
    h: '石棺中沉睡的教团圣女苏醒，与柠萌茶建立初识关系',
    kw: ['石棺', '圣女', '苏醒', '灵体对话', '教团'],
    imp: 8,
  },
  {
    c: '绮萝莉娅对柠萌茶的冒犯感到愤怒，但柠萌茶以她身体无法动弹为由拒绝道歉，自称是她的主人。他介绍了瓦伦蒂亚城的所在地。绮萝莉娅震惊于自己被转移到大陆南部，距教团所在的无尽树海极远，时间已过去29年。两人发生口角，柠萌茶让绮萝莉娅明白自己已无依靠。',
    h: '柠萌茶与绮萝莉娅初次交锋确立主从关系',
    kw: ['主从关系', '瓦伦蒂亚城', '时间流逝', '冲突'],
    imp: 6,
  },
  {
    c: '绮萝莉娅改变策略用撒娇方式要求柠萌茶保护她的身体，建议对外伪装成炼金人偶。柠萌茶询问教团之事，她承认是教团最强圣女，但表示接受宠物身份，只要柠萌茶答应保护条件。两人关系开始缓和。',
    h: '绮萝莉娅接受宠物身份，与柠萌茶达成初步契约',
    kw: ['契约', '人偶伪装', '宠物', '教团'],
    imp: 7,
  },
  {
    c: '柠萌茶确认绮萝莉娅是教团最强者。绮萝莉娅表示从未想过当教主。柠萌茶带她去浴室清洗时，绮萝莉娅透露无法感知身体触觉，但身体对语言指令有反应。柠萌茶对这种灵肉分离的现象十分好奇。',
    h: '确认实力背景，发现身体对语言反应的现象',
    kw: ['教团最强', '身体反应', '浴室', '灵肉分离'],
    imp: 7,
  },
];
const memories = [];
for (let t = 2; t <= 5; t++) {
  const tpl = memTemplates[t - 1];
  if (!tpl) continue;
  memories.push({
    id: 'mem-turn-' + t,
    saveId: SAVE_ID,
    createdAt: NOW + t * 60000,
    realTimestamp: NOW + t * 60000,
    timeRange: {
      start: `复兴纪元488年-08月-15日-${turnTimes[t - 2].h}:${String(turnTimes[t - 2].m).padStart(2, '0')}`,
      end: `复兴纪元488年-08月-15日-${turnTimes[t - 1].h}:${String(turnTimes[t - 1].m).padStart(2, '0')}`,
    },
    content: tpl.c,
    hiddenLine: tpl.h,
    keywords: tpl.kw,
    relatedCharacterIds: ['char-player-001', 'char-npc-qll'],
    importance: tpl.imp,
  });
}
...
const chatSession = {
  id: 'chat-test-progressive',
  name: '测试对话: 发现绮萝莉娅 (5轮)',
  messages,
  characterName: '柠萌茶',
  userName: '柠萌茶',
  presetId: null as string | null,
  lorebookIds: ['system_core', 'race', 'faction', 'character'],
  variables: JSON.parse(JSON.stringify(baseVars)),
  createdAt: NOW,
  updatedAt: NOW,
};
...
const saveSlot = {
  ...
  createdAt: NOW,
  updatedAt: NOW,
  activeSnapshotId: 'snap-turn-5',
  snapshots: [] as any[],
  metadata: {
    characterName: '柠萌茶',
    userName: '柠萌茶',
    gameStartTime: '复兴纪元488年-08月-15日-20:13',
    totalTurns: 5,
    description: '柠萌茶发现石棺中的教团圣女',
  },
};
...
const withEffects = (arr: any[]) =>
  arr.filter((x) => x.effects && Object.keys(x.effects).length > 0).length;
const withScripts = (arr: any[]) =>
  arr.filter((x) => x.scripts && Object.keys(x.scripts).length > 0).length;
...
// 装备 = inventory 中 equippedSlot 非空的物品（规范 §3），不再有独立的 equipment[]
const equipped = [...player.inventory, ...qll.inventory].filter((i) => i.equippedSlot);
console.log('Equipment with effects:', withEffects(equipped), '/', equipped.length);
console.log('Equipment with scripts:', withScripts(equipped), '/', equipped.length);
console.log(
  'StatusEffects with effectDescriptions:',
  qll.statusEffects.filter(
    (s) => s.effectDescriptions && Object.keys(s.effectDescriptions).length > 0,
  ).length,
);
