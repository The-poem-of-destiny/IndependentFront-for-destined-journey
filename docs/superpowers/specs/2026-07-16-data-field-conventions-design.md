...
>
> **M3 剩余过渡（M4 拆除）**: ① `const key = r.name ?? r.id` 六处过渡读（agent-orchestrator characters.replace/delta/add/remove + request_dispatcher）— M4 改 prompt 后删 `?? r.id` ② resolveCharacter UUID 兜底 — M4 删（铁律1 收口）③ agent-config.json 中 vars_update/dispatcher/item_gen/char_gen prompt 的 id 键示例 → M4 改为 name。
>
> **M4 代码侧收口执行注记（2026-07-17）**: 上条 ①② 已拆除——① agent-orchestrator Stage3 characters.replace/delta/add/remove 四处 `key = name ?? id` 过渡读删除（实际代码中为四处，dispatcher 侧无此形态；key 只认 name，缺 name warn+跳过），equipment 分支 `?? value.itemId` 过渡读同批拆除（原 itemId 语义已废）② resolveCharacter UUID 兜底删除（解析顺序收敛为: 名字精确匹配 → 主角/玩家别名 → throw 角色不存在），remove/rename_character 的跨存档守卫随兜底拆除转为死路径，同批移除，getCharacter 依赖清除。测试: state-manager.test 37 处按 id mock 改为按名寻址（getCharacters mock），3 个兜底用例改为负向断言（id 寻址必须报 角色不存在 且 getCharacter 不被调用）；agent-orchestrator.test 数据 id→name 键，新增 id-only 条目全跳过回归用例。验证: typecheck 0 错误，2754/2755 通过（仅既有 create-store 命定之灵 1 失败）。③ 由 prompt 对齐任务并行完成（vars_update 操作语义表 id 示例已改 name；memory_recall/plot 双检的 `{"id":}` 为 memory/plotEvent 合法账务键，保留）。残留小项: memory_summary 示例 relatedCharacterIds 仍用 `player_1`（memory 实体章范畴，非 characters.* 寻址，登记待清）。
