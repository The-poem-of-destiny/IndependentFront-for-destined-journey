...
>
> 状态：✅ **已实施（2026-08-12 合入，UI 真机走查过；AI 轮次待日常游玩验证）**。
> 2026-08-11 grilling 会话逐条走完设计树（12 问 + 数据集选择），
> 全部裁定记录在 §12；领域词汇已入根目录 `CONTEXT.md`「地图系统」节。
> 合入后有两次修订，都在 2026-08-13 同日（见 §6.2 末尾那段引注）：
> ①`travelRules.modes[]` + pack v1.1.0；②**出行方式降为纯展示参考行**（各方式天数并排、不可选，
> 不进出发指令 / 不进寻路状态 / 不进存档）。
...
> 倍率**——不进寻路状态、不进存档、不进 MAP_CONTEXT。**同日修订二**：方式行是纯参考
> 展示（各方式天数并排、不可选），**也不进出发指令**——指令保持基线「约 N 天」，
> 要坐什么玩家在输入框自己写（指令本就是可编辑文本，不替玩家做决定）。
> 旧包（v1.0.0，无 modes）coerce 成空数组，UI 不显示方式行，行为与从前逐字一致。engine 侧 `embarkCost` 单位澄清为 **km 等效**
