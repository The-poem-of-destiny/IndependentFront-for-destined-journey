...
>
> 📌 **2026-09-15 双角色会话定案**：本文的「整场一个持久会话」已成为历史设计。
> 现行实现仍是**一个 Kernel**，但模型侧固定使用 `combat_v3` 主持人与 `combat_enemy` 敌方决策
> 两个隔离的持久会话；玩家输入只进主持人，敌方只接收裁剪投影并只获当前敌方 actor 的动态
> 行动权限。试运行开关与旧单主持人兼容路径已退役，普通动作不增加主持人审批 LLM 环。
> 战斗中的 `write_summary` 工具与摘要收集状态也已退役；终局事实由主持人专用调用直接产出正文。
>
...
> 📌 **2026-09-15 更正**：下表记录 2026-08-09 的实施历史。现行 `combat_v3` 为 6 个战斗工具
> 加 4 个只读查询，已删除 `write_summary`；`combat_enemy` 另有更窄静态白名单与调用级动态授权。
...
> 📌 **2026-09-15 更正**：下列步骤 1–4 仍适用；步骤 5 已改为内核终局后专门调用主持人，
> 直接取得总结正文，不再在行动决策中收集 `write_summary`。
...
> 📌 **2026-09-15 更正**：本节的玩家意图路径继续有效，但“与敌方共用 handle”已经退役；
> 玩家意图只进入 `combat_v3` 主持人会话，敌方行动只进入隔离的 `combat_enemy` 会话。
