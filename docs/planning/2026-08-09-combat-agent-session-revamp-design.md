...
>
> 📌 **2026-09-14 双角色会话更正**：本文的「整场一个持久会话」只描述
> `combatAgentSplitEnabled=false` 的兼容路径。开启新开关后仍是**一个 Kernel**，但模型侧改为
> `combat_v3` 主持人与 `combat_enemy` 敌方决策两个隔离的持久会话；玩家输入只进主持人，敌方
> 只接收裁剪投影并只获当前敌方 actor 的动态行动权限。普通动作不增加主持人审批 LLM 环。
>
