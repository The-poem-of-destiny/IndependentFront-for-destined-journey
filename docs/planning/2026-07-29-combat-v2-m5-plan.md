...
function enterCombat() { isInCombat=true; combatLog=[]; combatLastAction=null }
...
function exitCombat(result) { activeCombat=null; isInCombat=false; combatLog=[]; /* 摘要回注已有 */ }

// M5b:
const combatAwaitingInput = ref(false)
function submitCombatInput(text: string) { /* resolve pending resolver */ }
```
