...
| 3   | CI 缺 `vue-tsc`（.vue SFC 检查无兜底）+ 无分支保护                                 | 质量地板有洞                             | 🔴     | ✅ CI 已含 vue-tsc（分支保护另计） |
...
| 7   | `package.json` license=`ISC` 与文档 MIT 矛盾（description 乱码已修）               | 元数据无守卫                             | 🟢     | ✅ 已改 `"license": "MIT"`         |
...
## 4. CI：质量地板 `[📦已落地，且已超出本节所写]`

> **现状（2026-08-18 复核）**：`.github/workflows/ci.yml` 已拆成 types / quality / test 三个并行 job，
> 共**八道闸门**：`typecheck` + `typecheck:vue`（即 vue-tsc）+ `typecheck:tools` + `build` ｜
> `format:check` + `lint` + `knip:ratchet` ｜ `test:run`。本节下面的 yaml 是当时的补法，留作沿革。
> **本地一键跑齐这八道 = `npm run gates`**（它与 ci.yml 逐条对齐，改一边要同步另一边）。
...
✅ 已建 `.github/pull_request_template.md`（仓库里那份是现行真源，下面这段是当时的初稿），
把"提交前文档检查"变成强制勾选：
