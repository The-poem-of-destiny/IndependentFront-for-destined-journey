...
> 📌 2026-09-16 更正：凭据引用现来自 `apiEndpoints` 与 `imageApiConnections` 两张设备本地连接表；
> 前者容纳 LLM / Embedding / Reranker，后者容纳 NovelAI。相同规范化地址 + Key 跨用途、协议和页面
> 仍映射到同一 `credentialId` 桶。设置页保存改为按策略行事务更新，禁止从任一页面整表覆盖另一页面的策略。
...
> 📌 2026-09-16 更正：地址/Key 修改和删除的引用检查必须跨两张连接表执行。只有通用源与图像连接
> 都不再引用旧凭据时才删除孤儿策略；局部页面保存不得清除另一张表仍在使用的桶。
