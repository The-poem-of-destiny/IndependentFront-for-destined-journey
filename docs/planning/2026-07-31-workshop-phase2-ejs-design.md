...
| **ST 宏嵌在 EJS 代码块内**（`event.json` uid 358：`if ({{roll 1d100}} >= 100)`） | 要求宏先剥离、EJS 后编译，与 D1 顺序相反，且全仓无 `{{roll}}` 解析器 → 该条目**注定回退原文**（= 它今天的现状）。全语料仅此 1 条，不为它修订 D1；冒烟测试改用**已知回退白名单**断言（防新增回归），不用裸百分比 | 接受（实施期裁定）；将来若做 `{{roll}}` 解析器再议 |
