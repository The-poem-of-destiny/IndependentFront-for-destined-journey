...
> 状态：**拷问定稿（2026-07-31，五轮）**，对应决策已录 ADR-30（AGENTS.md 设计约定）· 契约词汇见根目录 `CONTEXT.md`
>
...
3. **切片 3**：renderWorldBookEntries（分层+错误隔离+编译缓存）+ 全语料冒烟
...
6. **切片 6**：真机 debug loop 走查（关注：回退率、缓存命中字节数前后对比、story 首包延迟、冰之歌跨回合链）
