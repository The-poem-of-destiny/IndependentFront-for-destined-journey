...
> 改入；两轮修订记录见 §11。
> **v1.3（2026-08-07 缩减裁定）**：主人裁定**不做开源发布**——本仓保持公开仅为让用户核验
> 代码无害，分离范围收窄至敏感内容本体（世界书正文/预设/提示词）。三仓拓扑、快照切仓、
> IP 专名清洗作废或缩水，剩余工作聚焦「用户把真实内容导入回来」。裁定全文与逐条影响见
> 紧接本头注的 v1.3 节；§0-§11 保留作 v1.0-v1.2 设计记录，与 v1.3 节冲突处以 v1.3 为准。
> 配套实施计划：`2026-08-05-content-engine-separation-implementation-plan.md`。

---

## v1.3 缩减裁定（2026-08-07）—— 现行范围，覆盖下文冲突处

> 主人裁定（2026-08-07）：**不做开源发布**。本仓保持公开的唯一目的，是让用户能核验
> 「这份代码不含有害逻辑」，不追求开源项目意义上的 IP 纯净度。分离范围收窄为
> **敏感内容本体：世界书正文 / 预设 / 提示词（及同级语料）**。

### 状态：分离本体已完成

波 0-4 已全部合入 master（PR #36-#45，CI 绿）。HEAD 已是占位态：15 本真实世界书、真实
agent-config（13 个 agent 提示词 + story 预设）、美化规则、目录数据、`regex-remote-snapshot`、
`reference/` 整树、agent-framework 语料、`worldbook-ejs-corpus.test.ts` 与四个内容工具脚本
全部离树（PR #45，-172,726 行）；占位内容落 `public/data/`（URL 不变）；守门
`tests/no-world-content.test.ts` 与契约 `tests/contract/pack-install.contract.test.ts` 已上线。

### 作废 / 缩水清单

| 对象                              | v1.2 原裁定          | v1.3 现裁定                                                                        |
| --------------------------------- | -------------------- | ---------------------------------------------------------------------------------- |
| D1 公开仓快照切仓                 | 全新建仓单次快照     | **作废** —— 不建新仓                                                               |
| D2 本仓转私有冻结                 | 转私有归档           | **作废** —— 本仓保持公开，仍是开发主战场                                           |
| D4 公开仓改名                     | ✋ 主人定名          | **作废**                                                                           |
| D45 快照卫生（点目录 triage）     | 逐目录 review 后放行 | **作废**（无快照可言；`.gitignore` 补 worktrees 已完成并保留）                     |
| T19 fixture IP 名中性化（D33）    | ~60 文件清洗         | **取消** —— IP 专名不在「敏感」定义内                                              |
| T20 docs/仓面清洗（D36/D37 等）   | L 级清单             | **取消**大部；仅「测试者迁移公告」随 R3 交付；三处语料级文档见「可选扫尾」         |
| T25 快照切仓                      | 主会话执行           | **作废**                                                                           |
| D3 私有内容仓                     | 全新私有仓           | **缩水** —— 默认扩建既有私有 Worldbook 仓（✋ 主人可另裁）                         |
| T21/T22 私有仓 scaffold + 四门 CI | L                    | **缩水** —— 内容归家 + 构建脚本本地跑，CI 化推迟                                   |
| D32 守门词表轴                    | 全树扫描（含 docs/） | **钉死为 `public/data/**` 面**（波 4 实现即此口径，不再扩面）                      |
| D38 跨仓契约 CI                   | 私有 CI 常驻 job     | **缩水** —— 契约测试保留（公开仓自带），改为本地按需跑（R3）                       |
| D35 授权五件                      | 五件处置记录         | **推迟** —— 原则不变：未解决项只阻塞对应内容进包；D10 黑名单与 D13 provenance 保留 |
| §0.2 十五条验收                   | 全表                 | **缩为下方七条**                                                                   |

### v1.3 验收（七条）

1. 工作树无敏感内容（世界书正文/真实提示词/预设/对局语料）——守门测试口径（✅ 波 4 已达成）。
2. 占位态零安装可玩：`npm run build && npx vite preview` 演示环路（代码已交付，真机待验）。
3. 显式内容态：占位/装包/需注意/错误四态可见（已交付，真机待验）。
4. 用户导入真实 pack 后行为与内容内置时代等价，首装零虚假冲突（真机待验）。
5. 恢复默认 = pack payload > 占位；升级 diff、卸载回落、存档 uid 迁移可用（真机待验）。
6. 真实 pack v1.0.0 构建产出，且**先于测试者更新**可用（§5.8 时序 —— HEAD 已占位态，此条现已生效）。
7. SiliconFlow key 已轮换（✋ 主人确认 —— 历史公开含 key，轮换是唯一补救）。

### 显式接受的残留（记录在案）

- **git 历史仍公开含全部敏感内容**（`git show 2afc23c:data/defaults/agent-config.json` 即得
  全部真实提示词；世界书、预设、`reference/` 语料同理；另有 4 个 fork 在外）。分离只发生在
  工作树；不切仓即永久。主人已知悉并接受 —— 「敏感内容不在树上」≠「拿不到」。
- 测试/文档仍含 IP 专名（人名/势力名/血脉名等）与设计信息 —— 世界的**名词**公开，**正文**不公开。
- AGENTS.md 的势力清单与 T1-T7 数值表按 D6 继续留守（数值与公式是引擎不是 lore）。

### 可选扫尾（✋ 主人另裁，不阻塞任何事）

按「敏感 = 正文级语料」口径，HEAD 上还有三处边缘件：`docs/ARCHITECTURE.md` 364 行起的
世界观半部、`docs/planning/2026-07-31-combat-v3-stress-test/`（真实对局转写）、
`docs/reference/narrative_context_example.md`（完整叙事范例）。要挪是小时级任务。

### 剩余工作：聚焦「导入回来」

引擎侧导入机制（provider / planner / 执行器 / 导入 UI / 内容态 / 契约测试）已全部就位；
剩余全部是**内容侧**工作：内容归家 → pack 构建器 → 真实 pack v1.0.0 + 分发 → 真机三走查。
编排见实施计划 §6（R1-R4）。⚠️ `_private-staging/` 只存在于执行 T17 的机器（gitignored）；
任何机器都可从 `2afc23c`（交换前最后 commit）完整恢复 `data/`、`reference/`、
`tests/agent-framework/` 等全部离树内容。
...
> 📌 **2026-08-30 更正（D9 开场 prompt）**：开场 user 消息不再输出「起源印记」区块，也不再要求
> story 描写核心苏醒或显现方式。玩家选择的 `system_core` 条目继续通过世界书通道独立注入，由条目自身
> 决定人格、能力与出场表现；开场消息只以自然叙述交接剧情、时间与人物既定事实，避免数据表式措辞
> 污染首轮及后续对话语气。开场、角色状态投影以及内容包内 Story / 制作叙事指令统一使用以角色姓名
> 指代主角的第三人称有限视角。
...
**v1.3**（2026-08-07 主人缩减裁定，非评审轮）：不做开源发布——本仓保持公开仅为代码透明，
分离范围收窄至敏感内容本体（世界书正文/预设/提示词）。D1/D2/D4/D45 作废，T19/T20/T25 取消，
D3/T21/T22/D38-CI 缩水，D32 词表轴钉死 `public/data/**` 面，§0.2 验收缩为七条。剩余工作重排
为 R1-R4（实施计划 §6），聚焦用户导入路径。git 历史敏感内容永久公开被显式接受（裁定全文见
文首 v1.3 节）。触发背景：波 4（PR #45）已合入，HEAD 达成占位态；`_private-staging/` 仅存于
执行机，恢复锚点 `2afc23c`。
