...
> 状态：**已实施（2026-08-20）；专项真机验收与自动化降级矩阵待补**。
> 日期：2026-08-09
>
> 📌 **2026-08-20 实施更正**：提交 `a441924aa622014d2d327836264e0d61fe23148a`
> 已接入生产首页。实际代码采用 D6 的 chart（墨纸反相）分支，并将原计划的多文件场景拆分
> 收敛为 `drift/runtime.ts` 单文件运行时；这两点是现状记录，不把实施倒推成主人对视觉方向的
> 追认。十主题 / 降级 / 生命周期 / 多宽高比专项验收仍在 `TODO.md`。
>
> 🔴 **历史裁定（2026-08-09）**：D6（light 主题反相成纸上刻印）当时尚未获准。原型已证明它技术上成立、也确实好看，
...
> 📌 **2026-08-20 实施偏差**：生产代码未按上表拆成 `index.ts` / `scene.ts` / `layers/*`，
> 而是集中在 `src/ui/components/home/drift/runtime.ts`，唯一工厂名为
> `createAstralDriftScene()`；设置项落在已拆出的 `ThemeSection.vue`，不是旧表中的
> `SettingsPage.vue`。目标提交也未新增上述两份专项测试，测试缺口仍待闭环。
...
> 📌 **2026-08-20 现状注记**：生产实现已经采用下列方向 1（chart / 墨纸反相）。这只说明
> 当前代码如何渲染，不表示 2026-08-09 的保留意见已被推翻；主人后续视觉反馈优先于本段历史方案。
...
> 📌 **2026-08-20 已落实**：`three` 已进入 `dependencies`，`public/licenses/three.txt` 与
> `THIRD-PARTY-NOTICES.md` 已随同提交补齐；下文保留为当时的实施前检查项。
