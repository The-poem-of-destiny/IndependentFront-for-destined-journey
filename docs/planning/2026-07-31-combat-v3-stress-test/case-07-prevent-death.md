...
const grievousProtection: EffectAutomaton = {
  ...
  intent: (ctx) => [
    ...
    // ③ 施加重创/倒地（"代价"）
    { kind: 'ApplyStatus', target: ctx.unit.id, status: '重创/倒地', duration: { kind: 'until_removed' } },
...
const standardHealingPotion: ItemEffectBundle = {
  ...
  intents: (ctx, target) => [
    ...
    { kind: 'RemoveStatus', target: target.id,
...
const fireball: SkillAutomaton = {
  ...
  intent: (ctx) => [
    { kind: 'DeclareAttack',
      attacker: ctx.attacker.id, defender: ctx.defender.id,
      damageType: 'energy',
...
const elementalStaff = {
  ...
  intent: [{ kind: 'AddModifier', modifier: { type: 'percentage', value: 0.15 }, slot: 'final_damage' }],
};
```
...
const cremationTruthBuff: EffectAutomaton = {
  ...
  intent: (ctx) => [
    // 真伤：不走装备增益、不走暴击、不走减免
    { kind: 'DealDamage',
      target: ctx.defender.id, amount: 1500,
...
const blockAction: ReactiveActionAutomaton = {
  ...
  intent: (ctx) => [
    { kind: 'AddModifier', modifier: { type: 'percentage', value: -0.8 }, slot: 'final_damage', scope: 'this_attack' },
