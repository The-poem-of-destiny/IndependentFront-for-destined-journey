...
> 状态：✅ **已实施（2026-08-05，图像 v1 全量合入）**。本文件从设计定稿转为设计与实施记录。
> 日期：2026-08-04（v1.1 同日：补第二轮 UX 决策 D33–D49，见 §2.1）
>
> **后续**：v2（ComfyUI 本地后端 + 提示词方言，第 7 内容面）另见 [`2026-08-08-comfyui-image-provider-design.md`](2026-08-08-comfyui-image-provider-design.md) —— 它是本文的续篇，**本文的裁定除非在那份文件里被明确推翻，否则继续有效**。
>
...
> 范围：**NovelAI 单家 provider + 情景插画单一用途** —— 这条**只约束 v1**，ComfyUI 本地后端在 v2 已落地。四家 provider 对比、分级路由、角色画像路径见 **附录 A**（已核准，v2 直接取用）。
...
## v1.2 修订：角色外貌双份定义 · 地点定义废除（2026-08-04，真机实验催生）

三方对照实验（`tmp/nai-live/appearance-experiment.live.ts`，9 张真图，同 seed 锁变量）暴露了 v1 预设模型的两个洞。以下决策是那次实验的直接产物。

### 实验结论（先看数据再看决策）

| 变体            | 换装跟得上？        | 永久改变跟得上？              | 身份稳不稳                                                     |
| --------------- | ------------------- | ----------------------------- | -------------------------------------------------------------- |
| A 固定预设      | ❌ 仍是初始那件白袍 | ❌ 长发无疤（还多了对精灵耳） | ✅ 稳（但稳在**错的**样子上）                                  |
| B 全 Agent 派生 | ✅                  | ✅（疤淡但在）                | ⚠️ 措辞漂移 `golden eyes`→`gold eyes`、`slender`→`slim figure` |
| C 混合          | ✅                  | ✅ 短发（疤没画出来）         | ⚠️ **第二阶段头发提前变短**                                    |

🔴 **贯穿三个变体的一条定律：锚里没钉住的属性，模型每张图都重新决定一次。** C 的头发在第 2 阶段就短了 —— 剧本里那一刀是第 3 阶段才砍的，没有任何标签要求变短，只是身份锚（`silver hair, golden eyes, slender`）与可变层都没提长度，于是"没说"＝"随便画"。B 的措辞漂移是同一件事在词汇层的表现：同一个人被三种说法描述，脸就不再是同一张脸。

### D56 · 角色定义存两份：初始定义（基线）+ 会话定义（可变）

- **初始定义（base）**：这个角色"本来长什么样"。全局、跨存档共用，与素材库同口径（删一个存档不该让别的存档的角色换脸）。用户可在设置页编辑。
- **会话定义（session）**：**按存档（会话）跟踪**，由出图 AI 随剧情更新。删存档连带删除。

**为什么必须是两份而不是一份可变的**：一份可变的会被幻觉逐步腐蚀且无法回头 —— 模型某次写错一个发色，之后每张图都从那个错的基线继续演化。两份的结构保证**基线永远干净**，会话那份怎么漂都能一键重置回初始。这也是"自动写入"能被接受的前提：写的是会话副本，不是真源。

**合并**：出图时 `session ?? base`（按属性槽逐项覆盖，不是整份替换 —— 见 D58）。

### D57 · 没有初始定义的角色：首次出图时由 AI 现建一份

角色是**可枚举的实体**（有名字、有状态卡、数量有限），所以"先有定义再出图"是可达的；缺的那些让 AI 在**第一次为该角色出图时**建一份，落进 base，之后照常走 D56。

🔴 **建的是 base 不是 session**：第一次见到这个角色时她的样子，定义上就是她的初始样子。写进 session 会让"初始定义"永远空着，D56 的干净基线也就不存在了。

### D58 · 定义按**属性槽**存，不是一整串文本

实验里 C 的头发提前变短，根因是拼字符串时"没提到"与"明确不变"无法区分。改成具名槽（发色 / 发长 / 瞳色 / 体型 / 服装 / 状态 / 表情）之后：

- 合并是**逐槽**的：会话层只覆盖它真的写了的槽，没写的槽保留 base 的值
- **永远不存在"没人说过"的槽** —— 每个身份槽在 base 里都有值，于是模型没有自由发挥的余地
- 每个 provider 的 dialect（`danbooru` / `prose`）各自决定怎么把这些槽渲染成提示词，一份结构两种渲染，不必手写两串会漂移的文本

**永久 vs 临时**：剪发、留疤这类**永久**改变写进会话定义的身份槽（之后每张图都继承）；淋湿、沾泥、换今天这身衣服这类**临时**状态只进本次出图，不落库。

### D59 · 废除地点定义，改由 AI 在出图时现写

**理由（主人 2026-08-04）：地点无法穷举。** 总能再往下找到一个子地点 —— 宫殿 → 宴会厅 → 宴会厅的盥洗室。给"宫殿"写一份定义，画盥洗室时它是错的；给每个子地点都写，那是永远写不完的表。

角色与地点在这件事上**根本不同**：角色是有限的实体清单，地点是无限的层级空间。所以角色值得存定义，地点不值得。

- `composePrompt` 不再查 `location:` 预设，地点由 `image_prompt` 侧链在场景串里现写（侧链**本来就收 `location` 字段**，改动只是删掉查表那一步）
- `ImagePresetKind` 收敛为只有 `character`
- 🔴 **代价说清楚**：同一个地点在两张图里不保证长得一样。这是接受的取舍 —— 一个画错的盥洗室比一个永远画不完的地点表更可接受。若日后要一致性，正解是**把 AI 这次写出来的地点串按地点名缓存进会话**（与 D56 的会话副本同一套机制），而不是回头去写穷举表。

---

## v1.3 修订：AI 永远碰不到基线（2026-08-05，代码审查催生）

### D60 · 撤销 D57 的「AI 现建基线」，改为即兴外貌只落会话层

**D57 原本的裁定**是：没有初始定义的角色，第一次为他出图时由 AI 现建一份**基线**（base），
理由是「第一次见到这个角色时她的样子，定义上就是她的初始样子」。

**它在结构上有三个洞**（PR #29 审查发现）：

1. **基线是全局的** —— A 周目里 AI 即兴出来的样子会成为 B 周目的定义。角色是复用的没错，
   但同名角色在另一周目里被引入成另一个人是完全正常的，而那份即兴没有任何标记说明它是
   机器写的。
2. **两个重置口都够不着它** —— `resetOne` / `resetAll` 只清会话表。AI 写歪的基线只能靠
   用户去设置页手改，而他并不知道那份东西是怎么来的。
3. **界面在承诺一件代码不做的事** —— `ImagePresetList.vue` 原文写着「剧情里的变化由 AI 记在
   **本存档的副本**里，初始设定不受影响」，`character-appearance-store.ts` 的文件头写着基线
   「只有用户能改」。两句都是假的。

**裁定（主人 2026-08-05）：没有全局基线的角色，AI 生成的可变基线只用于本会话。**

于是不变式变得**无条件**：**AI 一个字节都写不到 `imagePresets`。**

- 没有基线时，即兴外貌落 `characterAppearances`，差量基准是**全空**（`EMPTY_APPEARANCE`）
- 判定与合并集中在新模块 `character-appearance-resolve.ts`（纯函数叶子），四个消费方
  共用同一个答案：装配 / 侧链点名 / 正文缺预设提示 / 写入路由
- 装配表由 `buildEffectivePresets(presets, sessions)` 产出。🔴 它必须把**只有会话副本、
  没有预设行**的角色也合成进去 —— 漏掉他们，那份即兴外貌永远到不了提示词，表现为
  「AI 明明报了外貌，画出来还是每张一个样」
- 侧链点名（`charactersNeedingBaseline`）的判据加上会话副本：**报过一次就收声**。
  一直点名会让模型每张图把九个槽重新即兴一遍，点名本身成为漂移的来源
- **代价说清楚**：这类角色「整档重置」之后会回到没有外貌可用的状态，下一张图重新即兴。
  这正是想要的语义 —— 用户从没定义过他，就没有什么可回退到

### D61 · 「存为初始设定」：从即兴到用户拥有的唯一路径，且由人按下

D60 砍掉自动建基线之后，需要一个替代品，否则 AI 即兴出来的好结果无法跨存档保留。

设置页「视觉预设」卡新增一节 **「本档临时外貌（还没有初始设定）」**，列出所有
「有会话副本、没有预设行」的角色，每行两个动作：

- **存为初始设定** —— 把这份即兴外貌提升成全局基线（提升后删掉会话行，它已逐字成为基线）
- **重置** —— 单角色清掉

🔴 **这一节不是可选的装饰**：没有它，AI 给这些角色定成了什么样是**看不见**的（上面那张表
按预设行渲染），也没有单角色重置可按，只剩「全部重置」那把大锤。

### D62 · 全空的 `appearance` 等于没有 `appearance`

顺带修掉的一个**静默** bug（同一次审查）。设置页编辑器**总是**整份写回九个槽（D58：留空
即空值），所以「只填了外观标签框、九个槽没动」的预设带着一个**存在但全空**的 `appearance`。

`danbooruOf` 原先按 `preset.appearance !== undefined` 判，于是这条**用户明明填过**的预设
会产出空串，被当成「没有预设」丢掉 —— 不报错、不告警，只是那个角色在每一张图里都不像。

现在判据统一走 `baselineOf`（= 槽里**有内容**）。D58 的原话本来就是「有属性槽就以槽为准，
**没有才**退回 `dialects.danbooru`」，全空的槽就是「没有属性槽」。
