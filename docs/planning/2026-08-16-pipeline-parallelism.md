...
>
> 🔴 **2026-08-17 起本文已部分过时**：`890f3ec` 的**提交级缓存（CommitScope）**改写了本文描述的
> `commitChatState` 锁段模型（写窗口被拉成「整次提交一拍」，出口统一 flush），**本文所有行号锚点
> 一并失效**。写队列 / barrier / 侧链旁路的**结论仍然成立**，但现行约定以根 `AGENTS.md`
> 的 ADR-21 注记为准（含 `persistFocusQuest` / `persistNewsRead` 必须进 `withSaveWriteLock`
> 并在锁内重读新鲜 profile 那一条）。
