...
| **Remote-declaration layer, custom protocols, `fs.watch`** | **The remote-declaration half was overturned on 2026-08-17** and shipped as 远程素材 v1 — declarations are scanned from two purely local carriers (world-book entries under the `char-info-ejs-builder` convention, statically extracted; content-pack §14 `remoteAssets`) and mirror-synced into ordinary asset rows at startup. See the 远程素材 v1 entry in `docs/CHANGELOG.md`. Custom protocols and `fs.watch` remain out: no browser analogue; port eval §18 already says skip. |
