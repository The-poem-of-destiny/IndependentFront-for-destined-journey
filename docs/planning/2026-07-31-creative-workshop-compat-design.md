...
**已确认的取舍（历史口径，Phase 2 落地后已解除）**：Phase 1 装进来的世界书条目在 Phase 2 落地前**不会被求值**——条目正文里的 EJS 会原样进入 Agent 上下文。这**不是**新增的缺陷：当时内置世界书就在这么干（`event.json` 297 个 EJS 块、`system_core.json` 252 个）。工坊条目与内置条目一视同仁，不做特殊门禁（D6）——这条在 Phase 2 之后依然成立：两者走同一条 `renderWorldBookEntries` 求值路径。
