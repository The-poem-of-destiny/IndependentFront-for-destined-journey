...
| [步骤 7](roadmap.md) EJS 能力面 + Legacy  | Q-09、Q-10、Q-27       | 🔄   | 三条各自落地了不依赖真机的那半；剩下的都压在同一个前置上——**Legacy 退役需真机走查**（别名层共享源码要与它同批，否则失去 parity 这唯一的差分护栏）。〔2026-08-18 复核〕**前置仍未解除**：`ejs-backend.ts` 的 `LegacyBackend`（`new Function`）还在——生产由 `main.ts` 切 QuickJS 且装载/失败双向 fail-closed，Legacy 现在只服务单测的 wasm 启动成本（`script-backend.ts:16-17` 就地写明「与 ejs-backend 的一处刻意不同」）。`AGENTS.md` 进度表里**工坊 P2（EJS 沙盒）仍是「✅ 待真机」**；P3 / P4 的「真机已过」走的是社交面与上游对齐，不构成 P2 的走查。故 Q-09 / Q-10 / Q-27 三条的剩余项照旧压在同一前置上，状态不变                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| [步骤 8](roadmap.md) 巨石拆分第一刀       | Q-23、Q-19             | 🔄   | Q-19 两个 PR 全落；Q-23 未动——它要先拍板域边界（`EffectIntent` 的传递闭包 ~400 行，盲搬等于替主人做决定）。〔2026-08-18 复核 · 与 `890f3ec` 分层收口的关系〕**Q-23 未被它满足，状态仍 ⬜。** `890f3ec` 在根 `AGENTS.md` 设计约定里立的是**跨层单向契约**（前端 → 引擎，`src/sillytavern/**` 禁 import `../ui/*` / `@ui/*` / `vue` / `pinia`，type-only 亦禁）并配了两道机器闸门（`eslint.config.js:116-132` 的 `no-restricted-imports` + `tests/layering-gate.test.ts`）——那是**引擎↔前端**这条轴。Q-23 问的是**引擎内部**的域边界：`types.ts` 兼任常量/函数库，以及 `combat-v3/types.ts` 把依赖方向反转。两道新闸门的 pattern 里没有任何一条约束 `types.ts ↔ combat-v3/`，复核实测三项证据全部照旧：仍无 `types-combat.ts`（现有 `types-audio/content/image/map/random-events` 五份）、`types.ts:17` 的 `import type { EffectAutomaton } from './combat-v3/types'` 反向边原样在、主干**从 3716 行涨到 4167 行**（`890f3ec` 把 `CreatePreset` 从前端 store 迁进 `types.ts:1384` 修的是跨层反向依赖，代价正好落在 Q-23 抱怨的那根主干上）。审查要求的「先拍板 `EffectIntent` 域边界」这一步仍未发生 |
...
| `04804f9` | Q-25 ✅、Q-18 ✅         | 视图层拆分 + 设置真源类型化。`SettingsPage.vue` 3823 → 418 行（12 个分区**全部**成为一行子组件，共用外壳 `settings-chrome.css` 只有一份）；`StatusOverview.vue` 1411 → 1175（画像段 → `usePlayerPortrait` + `portrait-messages`）；12 张 per-Agent map → 一张 `agents` 表 + `UiSettings`（**type 不是 interface**）。类型当场查出三个「读了但从没人写」的键，其中 `embeddingEndpointId` 意味着记忆向量化从未运行。三处顺手修的真缺陷：`AppModal` 卸载不还原 `body.overflow`、`ApiSection` 多根导致 `.centered` 落空、4 条依赖 CVE                   |
| `1f087d2` | Q-21 ✅                  | 结算层去重 + 两处 live bug。**刀一（集群）已由 Q-04/Q-15 的删尸体覆盖**，两份死实现连同 `cluster-system.ts` 早已不在。**刀二**：`runDamagePipeline` 两处 17 字段调用 → `buildDamageInput`，`damageType` 冻进 `DamageRecomputeCtx`（格挡重算此前回落到攻击者基础档，改变守方类型减免）；两处手抄的 `initialDamage` 删掉改读管线产物。**刀三**：`craft-request.ts` 收口 15 字段装配，骰子在工具边界真掷（此前生产恒 d20=10，大失败不可达、优/劣势整条死规则），check 的骰带按请求指纹留给 settle。**刀四**：`craft-projection.ts` 承接 140 行文本面板 |
...
| [Q-23](findings-t5-layering.md#q-23)                                          | 审查要求"动手前先拍板域边界"。`EffectAutomaton` 的传递闭包（`EffectIntent` 及其下游）约 400 行，搬哪些、留哪些是设计决定。〔2026-08-18〕`890f3ec` 的分层收口是**另一条轴**（引擎↔前端），不解这一条；详见上方步骤 8 行的复核注 |
| [Q-29](findings-t5-layering.md#q-29)                                          | 审查原文："动手前需要一个 owner 定下 seam 位置"，且含一处要主人确认的行为变更 —— 端点列表一次 `run` 内算一次意味着**回合中途改设置不再立即生效**                                                                               |
| [Q-24](findings-t5-layering.md#q-24) / [Q-20](findings-t6-safety-net.md#q-20) | 两条都只是没排期，**不需要任何拍板**（Q-24 第一刀还是代码注释里预先授权过的）                                                                                                                                                  |
| [Q-28](findings-t5-layering.md#q-28)                                          | 主人已拍板「保留并深化」（见下），但形式化那一刀尚未排期                                                                                                                                                                       |
...
**Q-21 里藏着两处 live bug（本次复核发现，审查原文未点出严重度）。** 〔✅ 两条均已修，见下〕
审查把 Q-21 定级「中 / 重复」，但其中两刀是当下就在错的：

1. **制作检定在生产环境恒为 d20 = 10。** 〔✅ 已修：骰子改在工具边界真掷（`takeCraftTape`），
   装配收进 `craft-request.buildCraftRequest`；`craft_check` 的骰带按请求指纹留给同参数的
   `craft_settle` 取走，AI 看到的评级即落库结果。骰数由优/劣势决定（齐平 1 颗、优劣势 2 颗）——
   **不能**图省事一律掷 2 颗，那会让常规检定的 `length === 2`，把大失败判据换个姿势再打掉一次。
   6 条回归测试在修复前全红。〕`agent-tools.ts:673` 与 `:742` 都以
...
2. **格挡重算路径用了另一个伤害类型。** 〔✅ 已修：`damageType` 冻进 `DamageRecomputeCtx`，
   两处 `runDamagePipeline` 调用合并成 `buildDamageInput`，恢复路径一个字段都不回头读
   `attacker`。回归测试让技能声明真实伤害、攻击者基础档是物理，修复前 `DamageApplied.damageType`
   报 '物理' 且伤害多挨一道 12.5% 的类型减免。〕`attack.ts:227` 传 `ability.damageType`，
