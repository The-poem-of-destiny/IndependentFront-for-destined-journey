...
| [Q-25](findings-t5-layering.md#q-25) 余下部分                                                                             | 只剩 **Agent 分区**（约 1370 行）没拆。**阻塞已解**：Q-18 已落地，13 张 map 变成一张 `agents` 表且读写收进 `agent-settings.ts`，现在照 `settings/audio/` 的样子拆成 `settings/agent/` 目录即可，纯机械 |
