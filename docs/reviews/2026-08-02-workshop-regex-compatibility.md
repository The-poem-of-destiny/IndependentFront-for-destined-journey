...
- 未命中原文**始终**由宿主 Vue 文本节点渲染，不进 iframe，也不进 parent DOM 的 HTML 面；正则 CSS/布局够不到普通正文与其它命中。
- replacement 与捕获内容不消毒、不转义；**每一次富命中独占一个** `sandbox="allow-scripts"` iframe。代价是跨命中 DOM 查询不再兼容（持久协调仍可走 `regexStorage`）。
- **按 markup 作者分两档脚本策略**（`BeautifierFrameScriptPolicy`）：
  - 用户装过的规则（内置预设 / 自建 / 工坊）= `allow`，即上面那套全开契约，工坊兼容面不动。
  - 本轮**模型输出**合成的卡片（`<item_info>` / `<task_info>`）= `block`：CSP 只放行带 nonce 的宿主引导脚本，卡片自带的 `<script>` 与 inline handler 由**浏览器**拦掉（不做正则消毒，markup 原样保留），`connect-src 'none'`，且不注入共享 `regexStorage` 快照。样式/图片/字体/媒体照旧，卡片视觉不降级。
  - 分档理由：隔离框够不到宿主是一回事，**该不该给这条正文脚本执行 + 网络出口**是另一回事。模型正文会被世界书 / 角色卡 / 工坊文案里的注入牵着走，而 story 预设的卡片本来就只有 div/b/span + 内联 style，不需要脚本面。
...
6. **执行预算**：iframe 的权限隔离不等于 CPU 隔离。无限脚本循环仍可能阻塞渲染线程，工坊入口在完成预算方案和更广泛视觉走查前保持关闭。宿主侧的匹配阶段已经封顶：`findEligibleMatches` 的**越界重试分支**按扫描字符数记账（`MAX_OVERLAP_SCAN_CHARS_PER_RULE`），贪婪 pattern 撞上前一条规则的占位符时退化成「少匹配几处」而不是 O(n²) 卡死；正常命中不经过这个分支，长正文多命中的规则不受影响。
7. **每命中一帧的开销（未量化）**：每个富命中都是一份完整文档 + 引导脚本 + `ResizeObserver` + `MutationObserver` + `document.fonts.ready`。长对话里可以累积到几百个 frame，`loading="lazy"` 只省首绘、不省常驻内存；规则帧还各持有一份 `regexStorage` 快照。解封入口前应给出上限或视口外回收方案。
8. **inline 命中会断段（未量化）**：每个富命中都是 `display:block; width:100%`，所以一条本意「给句中某个词上色」的规则会把整段拆成三块。内置 22 条全是块级，暂时看不出来；公共语料 99 条尚未按这个维度审过。
9. **交互边界**：联网不等于授予浏览器宿主能力。form、popup、download、top navigation 与嵌套 frame 仍被 sandbox/CSP 阻断；这些边界不阻止规则使用已放行的资源与网络 API。
10. **纯正则项目不可启用**：存档启用信号要求项目至少有一个世界书条目，`entryUids=[]` 恒为未启用。语料中两个纯正则项目可以安装规则，但当前在任何存档都不可达。
11. **modal 不保证**：sandbox 没有 `allow-modals`；`alert/confirm/prompt` 不属于作者兼容面。语料中有 6 条词法命中，需另做 Chromium 真机确认。
...
补充门（2026-08-03 审查后）：模型帧的 nonce-only `script-src` / `script-src-attr 'none'` / `connect-src 'none'` / 不内嵌 `regexStorage` 快照，与规则帧仍全开的对照；`origin: 'rule' | 'model'` 在 renderer 侧真的映射到两档策略；越界重试封顶（去掉封顶后同一用例超时，装上后毫秒级）；`beautifierBuiltinDisabled` 语义迁移只剪掉旧语义下的空操作 id。
