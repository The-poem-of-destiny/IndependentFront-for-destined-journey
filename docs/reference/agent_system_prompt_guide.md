...
## Delta 会话：首轮完整 prompt 与后续增量（2026-08-23 起）

主 DAG 普通 chat/chatStream 已接入 **delta session**（`prompt-session-assembler.ts`）。它的含义：

- **首轮 / 重基线**：完全复用现有模板渲染——System Prompt + 上下文模板照旧拼出完整 prompt，
  只额外在首轮 user 触发消息（「继续」）后面追加一段 code 固定的增量协议说明，以及可选的
  `tailPrompt`。**首轮长什么样，完全由你现在配置的 System Prompt / template / 预设决定**，
  不需要为新机制改任何提示词。
- **后续轮**：不再重新渲染完整 prompt，而是**复用上一次实际发出的 wire messages**，只在末尾
  追加一条新的 user 增量消息，固定按顺序携带三块：
  1. `<context_delta>` —— 相对上一成功轮次的权威状态变化（角色/资源/物品/技能/状态/任务/好感/
     变量/时间/地图/剧情/记忆/历史等）。空表示状态未变化；**这是当前最新状态，以此为准**。
  2. `<turn_context>` —— 本轮玩家输入、随机事件候选、最近战斗与上游 Agent 输出。
  3. 可选的末尾指令（= `tailPrompt`，有配置才有）。
- 旧消息一律不重写、不删除；被编辑/删除/重排的历史会触发**重基线**（重新完整渲染一轮）。
- **不影响本文一切既有约定**：System Prompt 仍是核心指令真源、模板仍是首轮 prompt 的拼装顺序、
  占位符分类仍由代码固定。你平时怎么改提示词，照旧。

> 触发重基线的时机：会话不存在 / 应用重载、静态配置（模板、systemPrompt、预设、世界书可见面、
> `historyLayers`、`tailPrompt`、endpoint/model）变化、失败/取消/流中断、手动重新生成、已知
> 上下文预算不足、存档切换/销毁。重基线总是从当前状态完整渲染，不会回滚。
