...
```
① 界面切换            进首页 / 捏人页 → 按该界面的默认场景选曲（App.vue）
② 地点变化（游戏内主路径）  player.location 与上次选曲时不同 → 按地点选曲
③ AI 标记（可选）     story 输出 <play_audio> → 按它给的情境/情绪选曲
```

**① 界面切换**（`view-audio.ts` + `App.vue` 的 watch）解决的是"离开游戏页音乐既不换也不停"——退回首页还放着上一场战斗的曲子。
...
**曲库在 `App.vue` 装**（`audio.init()`，幂等）：首页也要出声，不能等进了游戏页才装库。`GamePage` 仍会再调一次，那时直接空转。

② 与 ③ 的关系如下。

②③ 都收口在 `GamePipeline.flushPendingAudio()`，都在 `refreshFromDb()` 之后执行：
...
- 用户在设置里关掉「场景配乐」后**三条来源全都不生效**（界面切换那条也归这个开关管——一个开关关掉全部自动换歌，不设第二个），音乐完全交回手动控制。关闭期间照样记录地点，重新打开时不会为"早就待着的地点"补播一次。
...
#### 免手势自动播放：浏览器里做不到（2026-07-27 定论）

**结论：本项目当前形态（dev server + 浏览器）下不存在"零手势自动出声"，这是平台约束，不是可以靠改代码解决的缺陷。** 不要再为此写规避代码。

用户激活是**页面级、一次性**的：任何一次点击或按键之后，整个页面生命周期都保持解锁。所以"无手势"真正只影响一次页面加载里的**第一次**，而且 `main.ts` 已经把监听提到应用启动，进游戏的那一下点击就会被捡走。剩下的空档只有一种——**直接落在游戏页且全程零交互**（刷新停在游戏页、外链直达）。
...
要判断当前解没解锁，代码里可查 `navigator.userActivation.hasBeenActive` 与 `AudioContext.state === 'running'`。
