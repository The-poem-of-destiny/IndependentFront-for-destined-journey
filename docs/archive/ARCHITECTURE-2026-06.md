...
> 🔴 **过期提示（2026-08-17 标注）：本文「第一部分：软件架构」的内容截止到 2026-06-13，已经不再反映现状。**
> 已知失准处包括：全景图里的 React 界面层、被列为响应式状态中心的 `sillytavern-store.ts`（早已删除，Store 由 Pinia 接管）、
> 战斗 v2 的 `combat-resolver.ts`（战斗 v3 M5 已删）、以及文末对 `src/ui/` 组件数量的统计。
> **任何结构性判断以根 `AGENTS.md` 加两份分册（`src/sillytavern/AGENTS.md`、`src/ui/AGENTS.md`）为准**，本文软件架构部分只作历史脉络参考。
> 「第二部分：世界观架构」不受此影响，仍然有效。
>
