...
> 📌 **10j 接线注记（2026-07-20）**: 本文的剧情设计已全量接线落地，实施细节与字段/寻址修正见 `docs/archive/planning/2026-07-19-plot-system-plan.md`。与本文差异要点: ① 主线大纲改在**捏人页**生成（非游戏首回合），一次 AI 调用出结构化 JSON + Code 转事件树；② 事件寻址 id→title（铁律1）；③ 新增雷点 tabooContent（仅大纲生成时注入）与重 roll 修改模式；④ hidden 事件 UI 层做「？？？」蒙层+剧透开关（用户内容控制权原则），DB visibility 字段只由剧情推进翻转。
