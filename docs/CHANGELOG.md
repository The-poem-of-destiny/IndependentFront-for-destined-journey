...
### 图像 v1.4 —— 标签词库（中文→danbooru 对照表，Agentic 检索）｜ ✅ 实施完成，待真机（2026-08-05）

用户带来一份 SillyTavern 世界书格式的标签对照表（`[场景]：温泉` / `- 温泉：onsen, hot spring`），
真实体量**几千条**。目标：导入成引擎内置的词库，喂给写标签的那个 AI，让它产出的 NovelAI 提示词
用词稳定。

**三个决定**（用户裁定，2026-08-05）：

1. **只做导入，不出厂内置** —— 词库是用户自己的资产，仓库不夹带。
2. **检索走 Agentic**：AI 看到一份**只有名字**的目录 → 调 `get_image_tags` / `search_image_tags`
   拿标签 → 自己组装。**不是**「Code 按关键词挑好塞进提示词」—— 那把「这张图该用哪几条」的判断
   交给了子串匹配，而那恰恰是 AI 比正则强的地方。
3. **目录一次列全**（按分类分组的全部名字），不做「先看分类、再展开」的两级懒加载：目录**逐字节
   稳定**因而落在 prompt cache 的稳定前缀里，多一次往返换来的省不回来。

**这推翻了一条既有 doctrine**：`image_prompt` 此前被明确设计成**非 Agentic**（`ImagePromptClient`
只有 `chat`，注释写着「它不查库、没有一件事需要工具调用」）。词库让第一句不再成立。改法是
**按条件分岔而不是全局切换**：有词库且客户端实现了 `chatWithTools` 才走多轮（`maxRounds: 4`），
否则退回单次补全 —— 没导入词库的用户付的 token 与 v1.3 逐字相同，老的测试替身也继续能用。
为一个可选增强让整张图失败，是把「没有词库」升级成「没有图」。

**转换器**（`image-tag-bank-import.ts`，纯同步、**一条也不抛**，照 `planInstall` 的先例）：
`comment` 的 `[方括号]` 出分类与名字、`key[]` 出检索别名、`content` 的 `- 名字：tag, tag` 出标签。
四个坑写进了断言：

- 🔴 **`,` 与 `/` 语义不同** —— 逗号是「同时成立」（`onsen, hot spring, steam`），斜杠是「同类候选」
  （`cat ears/fox ears/dog ears`）。压平之后两者长得一样，AI 会把六种耳朵一起写进提示词。
- 🔴 **标签自带的冒号不许剥** —— 剥 `温泉：` 前缀的判据是「冒号左边含中日韩字符**且**够短」。
  只看冒号会把 `rating:general` 与 NAI 的 `1.5::tag::` 拦腰截断，**且是静默的**：截出来的前半段
  仍是一个语法合法的标签串，只是意思全变了。
- 🔴 **BOM / 零宽 / U+FFFD 在导入期清掉并留痕** —— 样本 uid 2 的 content 结尾就是一个真 BOM。
  编码闸门扫的是 `data/` 与源码，**扫不到用户运行时导入的库**，这道关只能由导入器自己把。
- 🔴 **读不懂的那条记进 `notes` 跳过，不抛** —— 几千条手写语料里一定有没见过的写法，
  抛错等于让一个畸形条目否掉整本。导入报告按四类如实分列（跳过 / 存疑 / 修过 / 重名）。

**存储**：Dexie **v20** `imageTagBanks`，**全局**（与 `imagePresets` 同口径 —— 删一个存档不该让另一个
存档的出图失去词汇），整本一行、进 FullBackup（纯文本几百 KB，手工整理过的词库丢了补不回来）。

**UI**：图像分区第三张卡（提示词生成 → 出图 → **标签词库** → 视觉预设）。导入报告、
目录字符数（「每次出图都会发一遍」）、以及一个**用 `searchTagEntries` 本尊**的检索预览 ——
另写一份模糊匹配就成了安慰剂：预览看着好好的、真出图时 AI 查不到，且不会有任何报错。

**实施中逮到的一处**：`setEnabled` / `rename` 把 `banks.value` 里的行展开后直接交给 Dexie →
`DataCloneError`（structured clone 不认 Vue 代理），而我自己的 try/catch 把它吞成了一句
「写入失败」。`db-write.detach()` 正是为这条不变式存在的，**类型系统完全看不见它**。

**尚未真机**：整条链路（导入几千条的真实语料 → 目录进提示词 → AI 调工具 → 出图）只有单测。
目录规模、模型愿不愿意用工具、以及 prompt cache 的实际命中，都要等真机数据。
