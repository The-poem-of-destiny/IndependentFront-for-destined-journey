...
- 通用源按 `kind + protocol` 分型为 LLM / Embedding / Reranker；LLM 支持 OpenAI Chat、Gemini 与
  Claude Messages 原生协议，自定义 body 覆盖/JSON Pointer 省略、实际输出预算和连接 revision 共用一条入口。
- 新协议适配层归一化普通响应、SSE、usage 与工具调用，同时无损保留 Gemini `thoughtSignature` 和
...
- 删除 API 添加/编辑弹窗中的“开启思维链”选项，并从端点类型、密钥迁移、运行时映射与调试导出中移除对应字段。
- 通用 OpenAI 兼容请求不再自动注入 DeepSeek 专属的 `thinking` 或 `reasoning_effort` 参数，由所选模型与供应商自行决定推理行为。
- 保留响应中原生 reasoning 的解析与开发者诊断展示；它们属于兼容响应处理，不是 API 池开关。

### 2026-09-14 战斗 V3 主持人与敌方决策 Agent 切割｜公开仓已实施，真实战斗待验

- 保留持久 id `combat_v3` 作为战斗主持人，新增 `combat_enemy` 敌方决策；开启
  `combatAgentSplitEnabled` 后使用两份 client/messages/session，但两者共享唯一 Combat Kernel。
- 新增调用级权限与角色投影：按 battle/phase/current actor/command kind 拒绝越权或陈旧调用；
  敌方只见己方精确战斗信息，对玩家只见 HP 百分比、公开状态与已揭示技能，不接收玩家私有输入。
- 主持人负责玩家意图、开场、已提交事实演绎和终局总结；敌方只决定当前敌方单位，普通行动没有
  主持人审批模型环。空输出、工具耗尽与请求失败会暂停当前输入并提供重试/退出，不再静默 pass。
- 新增敌方端点继承与显式失效闭锁、设置页默认关闭开关、双角色活动文案及战斗暂停恢复面；关闭
  开关保持旧单主持人路径。公开默认配置、指纹、工具白名单与回归测试同步更新。

验证：2026-09-14 `npm run gates` 全绿（392 个测试文件、9,554 项通过、8 项跳过）；设置页桌面与
900×700 窄屏真机走查通过，双角色列表、默认关闭开关、启用态和敌方专用配置均显示正确。
正式内容 pack、真实 LLM 战斗与 usage 对照仍待执行，后续验收已登记 `TODO.md`。

📌 **2026-09-15 双角色方案全面迁移**：经方案确认，`combatAgentSplitEnabled` 试运行开关、
设置页“拆分主持人与敌方决策”按钮与 Coordinator 单主持人兼容分支已退役；所有战斗固定使用
`combat_v3` 主持人 + `combat_enemy` 敌方决策两份隔离会话。旧 localStorage 键会在加载和再次
持久化时清理；敌方端点继续执行显式失效闭锁。主持人 client 在开战时建立但不额外发送请求，
保证结构化玩家指令同样拥有结算演绎与终局总结；敌方 client 仍在首次敌方行动时惰性建立。
同时补严敌方自定义模板的数据源隔离，强制屏蔽玩家原始输入、剧情正文与最近对话，并修正
“已耗尽行动槽的存活目标不可被攻击”的授权误判。旧 `write_summary` 工具与会话摘要收集状态一并
删除，终局总结固定由主持人专用正文调用生成；公开默认主持人 prompt 已同步成新职责边界。
正式内容 pack 与真实战斗验收仍保留在 TODO。

验证：2026-09-15 `npm run gates` 全绿（395 个测试文件、9,588 项通过、8 项跳过）；设置页桌面与
900×700 窄屏真机复验通过，旧开关和空壳均不存在，浏览器控制台零错误。

📌 **2026-09-15 主持人提示词收口**：公开默认 `combat_v3.systemPrompt` 按开场、玩家意图、
内核结算演绎和终局总结四阶段重写；移除旧的双槽补满流程，保留玩家未指定的行动槽，
并明确工具接受声明不等于结算成功。敌方命令由战斗系统校验并交给唯一 Kernel 执行，
无需主持人审批；主持人只根据已提交事实写结果，不提前编造命中、死亡或胜负。
敌方提示词、上下文模板及模型参数保持不变。

📌 **2026-09-15 主持人上下文模板同步**：继续更新 `combat_v3.template`，保留世界设定、
战斗指令和参战方三区，按静态设定在前、开战背景在后的顺序组织；明确模板仅承载开战快照，
玩家意图、动态面板与结算事实由后续阶段消息追加。开战名单不覆盖后续加入或离场状态，
不再将战斗背景误作行动授权；敌方决策直交内核结算，主持人不代选、不审批。
本次不新增占位符，敌方配置与模型参数不变。

📌 **2026-09-15 敌方提示词与模板重写**：`combat_enemy.systemPrompt` 与 `template` 同步明确
玩家视角阵营标签、当前单位授权、受限查询与未知信息边界；决策依据可见战况和已知能力，
不将未提交计划当战果，不通过其他查询入口反推玩家私有信息。同批命令核对槽位与已知资源预算，
依赖前一动作结算的后续选择留到下一决策窗口；失败纠正与战术弃槽区分，不以自动结束回合掩盖错误。
模板保留原有四个占位符，静态设定在前，开战背景与阵容随后；动态面板仍由 Coordinator 裁剪注入。
不修改主持人配置、模型参数或战斗规则；正式内容包与真实 LLM 验收仍待执行。

📌 **2026-09-15 调试记录修复**：玩家从战斗就绪页延后点击开始时，现在会为整场战斗建立独立的
Debug Turn，并让 `combat_v3` / `combat_enemy` 的请求、响应、工具调用和重试共用该持久化账本；
成功、放弃、取消与异常路径统一关闭日志。此前这条路径在剧情回合结束后使用 `detached` id，记录会因
找不到对应 turn 被静默丢弃。新增成功、provider 失败与主动放弃三条生命周期回归断言。
验证：2026-09-15 `npm run gates` 全绿（395 个测试文件、9,582 项通过、8 项跳过）。

📌 **2026-09-15 敌方决策批次修复**：真机导出显示 `combat_enemy` 在同一决策中依次提交
`declare_attack → end_turn`，Provider 与单条工具翻译均成功，但 Coordinator 的整批校验把“任何包含
结束命令的多命令批次”一律拒绝，与 reducer 已支持的“攻击后结束回合”矛盾。现改为：
`EndTurn` 可作为攻击/动作后的最后一条命令，仅拒绝其后继续行动、多个结束命令或与
`Flee` 混用。真正的整批校验失败会把工具顺序与精确原因回馈到同一会话后再重试，
同时把 Coordinator 暂停原因回写到对应 `combat_v3` / `combat_enemy` 调用的 `error`，
避免导出日志出现“战斗暂停但 Agent error=null”的假成功记录。
验证：2026-09-15 `npm run gates` 全绿（395 个测试文件、9,585 项通过、8 项跳过）。
