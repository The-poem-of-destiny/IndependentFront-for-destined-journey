    ...
    for (const block of blocks) {
      ...
      for (const [name, re] of FEATURES) {
        ...
        list.push({
          ...
          from: entry.id,
          code: block,
          needsAsync: /\bawait\s/.test(block),
        });
...
function minimalCompileOk(content) {
  ...
  for (const p of parts) {
    ...
    else if (p.open === '=' || p.open === '-')
      lines.push(`__o.push(String(${p.raw || 'undefined'}));`);
...
function main() {
  ...
  if (!args.src) {
    ...
    console.error(
      '  或: node scripts/scramble-worldbook-ejs.mjs --transform <夹具路径> [--seed N]',
    );
    process.exit(1);
  }
  ...
  console.log(`✅ ${outPath}`);
  console.log(
    `   扫描 ${scanned} 条目 → 含 EJS ${entries.length} 条 / 片段 ${fragments.length} 个 / ${kb} KB`,
  );
