...
| 战斗 v2 | 战斗系统架构 v2 重构（管道+中间件+同构契约+6 大类效果对齐 #265160+buff 规则对齐 [状态规则]+19 event+Combat Agent+独立战斗面板+计算分工）。魔改不照抄世界书，趣味优先+代码兜底。架构: docs/reference/combat-system-architecture.md；计划: docs/planning/2026-07-28-combat-system-v2-plan.md。M1-M6 六批次，§十三 待确认清单已全收口。**M1 ✅**（emitChain+script-registry，130）**M2 ✅**（modifier 6 大类+buff 去重，~140）**M3 ✅**（管道版+19event+登神+HP红线，~80）**M4 🔄核心**（combat systemPrompt+13工具注册+executeCombatToolCall 独立通道(B方案)+combat-runner 跨回合循环+game-pipeline onCombatTrigger 接入+item_gen 6大类契约+校验纯函数 54测+combat-agent-api.md 接口规格文档；agent-tools 58测/combat-runner 6测；待真机验证） | ✅ M4 完成 ｜待 M5 前端面板/M6 真机 |
