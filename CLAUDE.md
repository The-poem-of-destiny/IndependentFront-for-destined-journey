...
│                                       #    ①注释必须纯 ASCII（chcp 65001 让 cmd 的字节偏移解析器错位，
│                                       #      注释片段会被**当命令执行** —— 修复前 stderr 940B，注释里的
│                                       #      findstr 真的跑了 3 次，而旁边就是一句 taskkill /F）
...
│                                       # 🆕 素材管理系统设计 v1.0（D1-D23 决策表 + 命名约定/命名不变式/
│                                       #    zip 契约/存储单层+注入缝/plan-execute 拆分/双回退链）← 改素材必读
│                                       #    已实现；渲染面 2026-07-29 接通（§15.9）；大画像/裁剪台 §15.10
│                                       #    （🔴 §15.10 的真机验证记录只对 `e818b61` 那一版有效，
...
│   │   ├── CharacterPortrait.vue    ← [素材] **顶对齐大画像位**。**纯呈现组件，画面上不放任何家具**
│   │   │                               (无旋钮/无相机徽章/无浮层)，也不碰 asset-store: 只吃 `framing` prop 就画，
│   │   │                               谁给的、要不要落库都与它无关 —— 拖滑块的实时预览因此是白拿的(传没落库的草稿即可)
...
│   │   ├── PortraitSettingsDialog.vue ← [素材] 画像的**唯一调节面** (`ad612d5` 新建): 取景 x/y/缩放三滑块 + 复位 + 更换图片
│   │   │                               🔴 **它同时是预览台，而预览就是 `CharacterPortrait` 本体**，不是另画一份近似效果 ——
│   │   │                               复制一份预览样式迟早与真身漂开，共用组件是"所见即所得"唯一可信的写法
│   │   │                               📌 为什么把旋钮从画像上收走: 旋钮与相机徽章都盖在图上，而旋钮弹出的浮层**还会盖住
│   │   │                               画像自己** —— 等于让用户一边调一边看不见调的结果
│   │   │                               🔴 落库防抖(300ms)，**欠账连 id 一起记**: 只记「脏了」、落库时现读 `props.assetId`，
│   │   │                               会在换角色/换图那一刻把**上一张**的取景写到**新那条**上。关窗与卸载都必须补掉欠账
│   │   │                               📌 换图**不在这里实现**，只发 `replace` 事件 —— 字节分流(图片进裁剪台 / mp4 直通)
│   │   │                               在调用方，而调用方本来就有同样一条路径；两处各写一遍就是两套会漂开的规则
...
│   │   │                               命中 `立绘`/`立绘bg` → CharacterPortrait **大画像**；只有 `头像` → 留 1:1 小方框
│   │   │                               (判据是**链上命中的那一档**不是"有没有图"：把一张脸的特写拉满整栏只会糊成一团)
│   │   │                               [身份条 `a2411f3`] 有大画像时 种族·身份·职业·生命层级·冒险者等级 **盖在画框顶端**；
│   │   │                               只有头像/无素材时退回自己一行。同一份数据**刻意写两处 DOM**: 盖在画上那份必须是画像槽的
│   │   │                               **后代**(点在字上才能冒泡到槽)，另一份必须在槽**外**(否则一行身份文字变成"点了弹文件框"的按钮内容)
│   │   │                               🔴 护读底与主题**无关**(scrim 恒黑 rgba→0 渐变、字恒浅 `#f0ebe1`): 底下是用户导入的图，深浅未知，
│   │   │                               用主题变量调色会让浅色主题在白图上彻底失效。用渐变不用实色条 —— 实色条会盖掉用户刚裁好的构图
│   │   │                               **全站唯一带导入入口的渲染位** —— 整块画像可点(点/Enter/Space)，**一个入口两种去处**:
│   │   │                               · 已是大画像(`立绘`/`立绘bg`) → 开 **PortraitSettingsDialog**(取景滑块 + 更换图片)
│   │   │                               · 无素材 / 只有头像 → **直接**开文件框(头像是圆形裁切的脸位，无取景可言，
│   │   │                                 弹一个只有「更换图片」可点的窗口纯属多一次点击)
│   │   │                               选中文件后再按**这份字节能不能过画布**分两条:
│   │   │                               · 图片 → 开 AssetCropEditor 裁剪台，一张源图烘出 `立绘`+`头像` 两行
│   │   │                               · mp4 → **不开**裁剪台(画布只取得到某一帧；D7 也不让视频落 `立绘`)，走 importForCharacter
│   │   │                               并**写死单个 `立绘bg`**(存进去的必须是确定的一格，只有读取才降级)
│   │   │                               🔴 为什么是 `立绘bg` 而不是 `头像`: 这个槽**读**的是立牌链，`头像` 在链的**最末** ——
│   │   │                               只要角色已有任一立绘，写头像就是"画面一动不动却弹一句已设为画像"。`立绘bg` 同为 D7 认可的视频落点
│   │   │                               🔴 写入定位到**一格**、读取走**一条链** → 落库后必须回读**这一格现在显示的是不是刚写的那一行**，
│   │   │                               只凭 store 返回 `'ok'` 就报成功是在骗人；被旧 `立绘` 压住时如实说"谁压着、去哪改"(永不覆盖 D11)
...
| 🖼 素材 | 三段式: ①导入条(一键 zip 导入素材+音频同一个包，按扩展名分流 / 进度+取消 / 结构化回执: 新增·跳过重复·编号·命名冲突·警告 / 配额 + `persist()` 结果) ②素材库(按角色分组卡+变体数+无主图徽章 / 全部素材扁平表 + 搜索 + 类型过滤 + shift 多选 + 批量删除 / mp4 走 `<video muted playsinline>`) ③变体抽屉(设为主图 / **裁剪**(对着已有素材重开 AssetCropEditor，一张源图重烘立绘+头像；三态里的「不生成」正是为这条路存在——重裁立绘时几乎从不想再铸头像) / 全字段改名+不变式行内拒收 / 删除)。**导入的素材游戏内会直接显示** —— 五个渲染位已接通(StatusOverview 玩家画像 / CreateStepConfirm / CharacterListPanel ×2 / ScenePanel NPC 立牌位)，按名字严格 `===` 取图、取不到照旧显示首字母；本分区仍是**唯一的整批导入与管理入口**，游戏内只有玩家画像位有一个定点导入口(点大画像 → **PortraitSettingsDialog** 调取景/换图；无素材或只有头像 → 直接开文件框。**画像上不放任何控件**，命中大画像时顶端只盖一条**身份条**，那是内容不是控件)。素材库全局共享不随存档；**不参与存档导出/导入**(走 zip 导出)，「清除全部数据」会一并销毁。⚠️ 首页 🧪 **快速测试**同样会清库(素材/音频一并没) —— 想留库请用旁边的「快速测试（保留数据）」 |
