...
beforeEach(() => {
  ...
  __setFolderTestHooks({
    ...
    saveHandle: async (rec) => { store.set(rec.id, rec); return rec.id },
    deleteHandle: async (id) => { store.delete(id) },
  })
})
...
describe('pickLibraryFolder', () => {
  ...
  it('用户取消（AbortError）返回 null 且不写库', async () => {
    ...
    err.name = 'AbortError'
    __setFolderTestHooks({ picker: async () => { throw err } })
...
describe('checkPermission', () => {
  ...
  it('方法抛错时当作 prompt', async () => {
    ...
    ;(dir as unknown as { queryPermission: unknown }).queryPermission = async () => { throw new Error('boom') }
  ...
  it('用 mode:read 调用 queryPermission', async () => {
    ...
    expect((dir as unknown as { queryPermission: ReturnType<typeof vi.fn> }).queryPermission)
...
describe('scanFolder', () => {
  ...
  it('子目录里的音频不会被递归收集', async () => {
    ...
    const dir = fakeDirHandle({
      entries: [Object.assign(sub as unknown as object, { kind: 'directory', name: 'sub' }), fakeFileHandle({ name: 'top.mp3' })],
    })
    const res = await scanFolder(dir)
