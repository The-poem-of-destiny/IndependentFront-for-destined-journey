...
describe('cropImageBlob', () => {
  it('把夹逼后的源矩形交给 drawImage，画布尺寸即输出尺寸', async () => {
    ...
    // x 被夹到 0，宽度相应缩成 50
    expect(rig.draws[0]).toMatchObject({ sx: 0, sy: 10, sw: 50, sh: 80, dx: 0, dy: 0, dw: 50, dh: 80 })
  ...
  it('🔴 零面积 → 抛 empty-rect，绝不静默产出一张空白图', async () => {
    const rig = makeRig(200, 300)
    await expectCropError(cropImageBlob(sourceBlob(), { x: 10, y: 10, w: 0, h: 50 }, rig), 'empty-rect')
    // 一个像素都没画、一个字节都没编
  ...
  it('造不出画布 → no-canvas', async () => {
    ...
    await expectCropError(cropImageBlob(sourceBlob(), { x: 0, y: 0, w: 10, h: 10 }, rig), 'no-canvas')
  })
