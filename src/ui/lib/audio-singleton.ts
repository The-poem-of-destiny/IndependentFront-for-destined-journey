...
import { AudioManager, AUDIO_DEFAULT_FADE_MS, type ManagerAudioContextLike } from '@engine/audio-manager'
import type {
  AudioBufferLike,
...
function createContext(): ManagerAudioContextLike {
  ...
  const Ctor = (g.AudioContext ?? g.webkitAudioContext) as (new () => ManagerAudioContextLike) | undefined
