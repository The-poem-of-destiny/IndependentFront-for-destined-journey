...
describe('浏览器工厂：真实全局可用时使用之', () => {
  ...
  it('存在 Audio 时构造元素并置 crossOrigin=anonymous', () => {
    ...
    expect((FakeAudioElement.last as unknown as Record<string, unknown>).crossOrigin).toBe(
      'anonymous',
    );
  });
});
...
describe('installUnlockListener', () => {
  ...
  it('幂等：重复调用只注册一对监听', () => {
    ...
    installUnlockListener();
    installUnlockListener();
    const types = add.mock.calls
      .map((c) => c[0])
      .filter((t) => t === 'pointerdown' || t === 'keydown');
