...
│   ├── image-tag-bank-store.ts      ← [图像 v1.4] 标签词库 Dexie 唯一读写口（全局，不随存档删）
│   │                                   🔴 出图链路只读 `enabledEntries` —— 两层启用开关（整本 / 条目）
│   │                                      只在 `collectEnabledEntries` 一处过。各处自己 filter 一遍，
│   │                                      就会出现「目录里印着的名字，工具查不到」，而那不会有任何报错
│   │                                   🔴 一条都读不出来时**不落库**：空词库在列表里没法解释自己
│   │                                   🔴 写库前必须 `detach()` —— 源对象来自 banks.value（Vue 代理），
│   │                                      Dexie 的 structured clone 不认代理，**类型系统看不见这条**
...
│   │       │                              写错框两边都不报错，只是画出来不对
│   │       ├── ImageTagBankCard.vue ← [图像 v1.4] 第三卡「标签词库」：导入 ST 世界书格式的
│   │       │                           中文→danbooru 对照表（几千条量级）
│   │       │                           🔴 导入报告**分四类如实列**（跳过/存疑/修过/重名，判定在
│   │       │                              同目录 tag-bank-report.ts 的纯函数里）—— 只报成功数会让
│   │       │                              用户以为整本都进来了。分类逻辑藏在文件选择器回调后面，
│   │       │                              留在组件里 jsdom 测不到，所以提成了纯函数
│   │       │                           🔴 目录字符数摆在明面上：目录每张图发一遍，逐字节稳定因而
│   │       │                              能命中 prompt cache，但那是**折扣不是免费**
│   │       │                           🔴 检索预览用 `searchTagEntries` **本尊**（与 search_image_tags
│   │       │                              工具同一个函数）—— 另写一份模糊匹配就成了安慰剂：
│   │       │                              预览看着好好的、真出图时 AI 查不到，且不会有任何报错
│   │       └── ImagePresetList.vue  ← 第四卡「视觉预设」：角色**初始设定**（属性槽）+ 本档变化
...
| 🖼 图像生成     | 四张卡：提示词生成（`image_prompt` 的 Agent 配置，存 `agents` 袋子）/ 出图（三档开关 + NAI 参数 + 免费额度指示，存 `UiSettings`）/ 标签词库（导入的中文→danbooru 对照表，存 Dexie `imageTagBanks`）/ 视觉预设（角色初始设定存 Dexie `imagePresets`；本档外貌存 `characterAppearances`，含「存为初始设定」） |
