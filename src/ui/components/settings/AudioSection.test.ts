...
describe('AudioSection', () => {
  ...
  it('未选文件夹时提供「选择音乐文件夹」并调用 pickFolder', async () => {
    ...
    const btn = wrapper
      .findAll('.folder-strip button')
      .find((b) => b.text().includes('选择音乐文件夹'));
  ...
  it('missing 曲目渲染「文件已移除」徽章且试听被禁用', async () => {
    ...
    const audition = row
      .findAll('button')
      .find((b) => b.attributes('aria-label')?.includes('试听'));
  ...
  it('删除曲目先弹确认，取消则不落库（曲库段 → 弹窗助手）', async () => {
    ...
    const cancel = [...document.body.querySelectorAll('.modal-footer button')].find((b) =>
  ...
  it('曲目行上只留「移出列表」，不再提供 ▲▼ 逐格移动', async () => {
    ...
    const labels = playlistRows(wrapper)[0]
      .findAll('button')
  ...
  it('无选中时批量按钮 disabled', async () => {
    ...
    const del = wrapper
      .findAll('.batch-bar button')
      .find((b: any) => b.text().includes('删除选中'));
  ...
  it('批量加入播放列表走 addTracksToPlaylist，跳过数如实播报', async () => {
    ...
    const spy = vi
      .spyOn(store, 'addTracksToPlaylist')
      .mockResolvedValue({ ok: 1, skipped: 1, failed: 0 });
    const boxes = wrapper.findAll('.row-check');
    await boxes[0].trigger('click');
    await boxes[1].trigger('click', { shiftKey: true });
