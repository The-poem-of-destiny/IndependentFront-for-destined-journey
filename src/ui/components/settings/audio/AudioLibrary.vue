...
const selectedMusicCount = computed(
  () => selectedTracks.value.filter((t) => t.kind === 'music').length,
);
...
const allFilteredSelected = computed(
  () =>
    ...
    filteredTracks.value.every((t) => selectedIds.value.has(t.id)),
);
...
function buildBatchDeleteMessage(list: AudioTrack[]): string {
  ...
  if (blobCount > 0) {
    lines.push(
      `${fileCount > 0 ? '另有' : '其中'} ${blobCount} 首存放在浏览器存储中，删除后不可撤销。`,
    );
  }
  if (builtinCount > 0) {
...
async function onFilesPicked(e: Event): Promise<void> {
  ...
  try {
    ...
    emit('announce', created.length > 0 ? `已添加 ${created.length} 个音频` : '没有音频被导入。');
  } catch (err) {
    if (isQuotaError(err)) {
      ui.toast(
...
<template>
  ...
  <div class="lib-upload">
    ...
    <AppButton variant="primary" size="sm" @click="pickFiles"
      ><i class="fa-solid fa-plus" aria-hidden="true" /> 上传音频</AppButton
    >
    <input
      ref="fileInput"
      class="file-input"
  ...
  <div class="lib-toolbar">
    ...
    <label class="reveal-label">
      <input v-model="showHiddenBuiltins" type="checkbox" />
  ...
  >
    ...
    <span class="src-text" :title="sourceHint(t)"
      >{{ sourceLabel(t) }} <span class="sr-only">{{ sourceHint(t) }}</span></span
    >
    ...
    <template v-if="t.builtin">
      <button
        class="icon-btn"
        :aria-label="isHidden(t) ? '取消隐藏' : '隐藏内置曲目'"
        @click="toggleHideBuiltin(t)"
      >
