...
function maskKey(key: string): string { if (!key || key.length < 8) return key ? key.slice(0,3)+'***' : ''; return key.slice(0,3)+'***'+key.slice(-4) }
	...
	async function fetchModelList() { apiFormFetchingModels.value=true; const rk=apiForm._realKey||apiForm.apiKey; try{const r=await new Promise((ok,rej)=>{const x=new XMLHttpRequest();x.open("GET",apiForm.baseUrl.replace(/\/+$/,'')+"/models");x.setRequestHeader("Authorization","Bearer "+rk);x.timeout=10000;x.onload=()=>{if(x.status>=200&&x.status<300)ok(x);else rej(new Error(x.status+" "+x.responseText.slice(0,100)))};x.onerror=()=>rej(new Error("network error"));x.ontimeout=()=>rej(new Error("timeout"));x.send()});const d=JSON.parse(r.responseText);const _rawModels=(Array.isArray(d)?d:Array.isArray(d.data)?d.data:Array.isArray(d.models)?d.models:[]).map(function(m){return typeof m==='string'?m:m.id}).filter(Boolean);apiModels.value=[...new Set(_rawModels)];console.log('[fetchModelList] raw response:',JSON.stringify(d).slice(0,800),'→ unique models:',JSON.stringify(apiModels.value));if(apiModels.value.length>0 && !apiForm.model){if(apiForm.apiType==='embedding'){const emb=apiModels.value.find((m=>m.toLowerCase().includes('embedding')));apiForm.model=emb||apiModels.value[0]}else{apiForm.model=apiModels.value[0]}}ui.toast("got "+apiModels.value.length+" models","success")}catch(e){const msg=(e.message||"").slice(0,100);const hint=msg.indexOf("401")>=0?"（Key 无效或与端点不匹配，如 z.ai coding key 必须配 coding endpoint）":msg.indexOf("404")>=0?"（该端点可能不支持 /models，请手动填 model）":msg.indexOf("network")>=0?"（代理或网络问题）":"";ui.toast("获取失败: "+msg+hint,"error")} apiFormFetchingModels.value=false }
