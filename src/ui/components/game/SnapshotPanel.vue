...
async function fetchSnapshots() {
  if (!game.activeSaveId) { snapshots.value = []; return }
  try {
...
async function restore(snap: Snapshot) {
  if (game.isInCombat) { errorMsg.value = '战斗进行中，无法恢复'; return }
  if (currentSnapshotId.value === snap.id) return
  const ok = window.confirm(`恢复到「第 ${snap.turn} 回合」？\n该回合之后的所有进度（对话 / 状态 / 记忆）都会被丢弃。`)
  if (!ok) return
...
watch(() => game.activeModal, (m) => { if (m === 'snapshots') fetchSnapshots() })
</script>
...
<template>
  <div class="snapshot-panel">
    ...
    <div class="combat-warn" v-if="game.isInCombat">⚠ 战斗进行中，恢复已禁用</div>
    ...
    <div class="snapshot-list" v-if="snapshots.length > 0">
      ...
      >
        <div class="snap-header">
          ...
          <span class="snap-current" v-if="currentSnapshotId === snap.id">当前</span>
        </div>
        <div class="snap-meta">
    ...
    <div class="empty" v-else>暂无快照（对话几轮后会自动产生）</div>
    <div class="error" v-if="errorMsg">{{ errorMsg }}</div>
  </div>
</template>

<style scoped>
.snapshot-panel { display: flex; flex-direction: column; height: 100%; padding: 8px; gap: 8px; }
...
.panel-hint { font-size: 0.6875rem; color: var(--theme-text-muted); }
.combat-warn { font-size: 0.75rem; color: var(--theme-danger, #e5484d); padding: 6px 8px; background: var(--theme-surface-muted); border-radius: var(--theme-radius-sm, 4px); }
.snapshot-list { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 6px; }
.snapshot-card { background: var(--theme-card-bg); padding: 8px 10px; border-radius: var(--theme-radius-sm, 4px); border: 1px solid var(--theme-card-border); }
.snapshot-card.current { border-color: var(--theme-primary); }
.snap-header { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; }
.snap-turn { font-size: 0.8125rem; font-weight: 600; color: var(--theme-text-primary); }
.snap-reason { font-size: 0.625rem; padding: 1px 6px; background: var(--theme-surface-muted); color: var(--theme-text-muted); border-radius: 3px; }
.snap-current { font-size: 0.625rem; padding: 1px 6px; background: var(--theme-primary); color: var(--theme-on-primary, #fff); border-radius: 3px; margin-left: auto; }
.snap-meta { display: flex; gap: 10px; font-size: 0.6875rem; color: var(--theme-text-secondary); margin-bottom: 2px; }
.snap-realtime { font-size: 0.625rem; color: var(--theme-text-muted); font-family: monospace; }
.snap-actions { margin-top: 6px; }
.empty { padding: 24px; text-align: center; color: var(--theme-text-muted); font-size: 0.8125rem; }
.error { font-size: 0.75rem; color: var(--theme-danger, #e5484d); }
</style>
