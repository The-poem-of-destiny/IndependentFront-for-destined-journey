...
<template>
  <div class="ci-card" :style="{ '--ci-tier': tierColor, '--ci-race': raceColor }">
    ...
    <div class="ci-body">
      ...
      <div v-if="event.details.personality || event.details.appearance || event.details.likes || event.details.clothing" class="ci-section ci-section-divider">
        ...
        <div v-for="group in inventoryGroups" :key="group.type">
          ...
          <div v-for="inv in group.items" :key="inv.name" class="ci-item-card ci-accent-race">
            <div class="ci-item-header">
              ...
              <span v-if="inv.rarity" class="ci-item-subtitle" :class="qualityGlowClass(inv.rarity)">{{ inv.rarity }}</span>
            </div>
      ...
      <div v-if="event.details.equipment?.length" class="ci-section ci-section-divider">
        ...
        <div v-for="eq in event.details.equipment" :key="eq.name" class="ci-item-card ci-equip ci-accent-race">
          <div class="ci-item-header">
            ...
            <span v-if="eq.quality" class="ci-item-subtitle" :class="qualityGlowClass(eq.quality)">{{ eq.quality }}</span>
          </div>
          <div class="ci-item-body">
      ...
      <div v-if="event.details.ascension?.enabled" class="ci-section ci-section-divider">
        ...
        <!-- 登神路径 path -->
        <p v-if="event.details.ascension.path" class="ci-dao-path">{{ event.details.ascension.path }}</p>
        ...
        <div v-if="event.details.ascension.divineKingdom?.name" class="ci-item-card ci-accent-tier">
          ...
          <div class="ci-item-body">
            <p v-if="event.details.ascension.divineKingdom.description" class="ci-item-desc">{{ event.details.ascension.divineKingdom.description.slice(0, 200) }}</p>
          </div>
        </div>
          ...
          <div v-for="law in event.details.ascension.laws" :key="law.name" class="ci-item-card ci-accent-tier">
            ...
            <div class="ci-item-body">
              ...
              <div v-if="law.passiveEffects?.length" class="ci-effects">
                <span v-for="pe in law.passiveEffects" :key="pe" class="ci-effect-pill ci-eff-passive">
                  <span class="ci-effect-key">被动</span>
                  <span>{{ pe }}</span>
                </span>
              </div>
              <div v-if="law.activeEffects?.length" class="ci-effects">
                <span v-for="ae in law.activeEffects" :key="ae" class="ci-effect-pill ci-eff-active">
...
.ci-profile-grid { display: flex; flex-direction: column; gap: 8px; }
.ci-profile-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.ci-profile-cell { min-width: 0; }
.ci-story {
  white-space: pre-line;
  line-height: 1.6;
  font-size: 0.75rem;
...
.ci-eff-passive { border-color: rgba(53,201,138,0.3); }
.ci-eff-active { border-color: rgba(245,158,11,0.3); }
</style>
