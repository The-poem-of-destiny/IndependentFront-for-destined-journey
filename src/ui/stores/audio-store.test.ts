...
vi.mock('@engine/database', () => ({
  ...
  deleteAudioPlaylist: vi.fn(async (id: string) => {
    playlistRows.delete(id);
  }),
...
function fakeDirHandle(files: FakeFile[], opts: { permission?: string; name?: string } = {}) {
  ...
  return {
    ...
    async getFileHandle(name: string) {
      ...
      return {
        async getFile() {
          return { name: f.name, size: f.size, type: f.type, __blob: true };
        },
      };
    },
...
describe('audio-store · loadBlob 分派', () => {
  ...
  it("source='builtin' 不碰文件夹，仍走原路径（返回 undefined）", async () => {
    ...
    storedHandle(handle);
    const getFileHandle = vi.spyOn(
      handle as unknown as { getFileHandle: (n: string) => unknown },
      'getFileHandle',
    );
...
describe('audio-store · 扫描对账', () => {
  ...
  it('单条落库失败不中断对账，其余曲目照常刷新 + 汇总上报一次', async () => {
    ...
    storedHandle(
      fakeDirHandle([MP3('a.mp3'), MP3('b.mp3'), MP3('c.mp3')], { permission: 'prompt' }),
    );
...
describe('audio-store · 名字唯一性（仅新写入）', () => {
  ...
  it('上传中间一条失败不中断整批，其余照常导入 + 汇总提示一次', async () => {
    ...
    const created = await store.uploadFiles([
      audioFile('a.mp3'),
      audioFile('b.mp3'),
      audioFile('c.mp3'),
    ]);
  ...
  it('文件夹扫描撞名自动编号，两行都在', async () => {
    ...
    storedHandle(
      fakeDirHandle([MP3('night.mp3'), { name: 'night.wav', size: 20, type: 'audio/wav' }]),
    );
    const store = useAudioStore();
    await store.init();
    ...
    // 扫描永不跳过文件
    expect([...trackRows.values()].map((t) => t.relativePath).sort()).toEqual([
      'night.mp3',
      'night.wav',
    ]);
  });
