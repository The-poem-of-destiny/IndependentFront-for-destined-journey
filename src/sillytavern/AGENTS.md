...
src/sillytavern/                    ← 核心引擎
  ...
  │   └── v20+: imageTagBanks（标签词库，图像 v1.4）——**全局**，与 imagePresets 同口径：
  │              词库是参考资料，删一个存档不该让另一个存档的出图突然失去词汇。整本一行
  │              （`entries[]` 内联，同 worldBooks）；只索引 id 与 enabled，条目级检索在内存里
  │              做（几千条别名的子串匹配是毫秒级的事，建索引只会让导入变慢）。**进 FullBackup**
  │              —— 纯文本几百 KB，与图片字节不同不会爆炸，而手工整理过的词库丢了补不回来
  ...
  │                                    🔴 **v1.4 起它可能是 Agentic 的**：此前这里写着「image_prompt 是
  │                                       普通补全，不查库、没有一件事需要工具调用」—— 标签词库让第一句
  │                                       不再成立。**有词库**且客户端实现了 chatWithTools 时走多轮
  │                                       （maxRounds 4）；**没词库**时一个 schema 都不发，与 v1.3 逐字
  │                                       相同。两个条件缺一就退回单次补全，**不是报错** —— 为一个可选
  │                                       增强让整张图失败，是把「没有词库」升级成「没有图」
  ├── image-tag-bank-import.ts      ← [图像 v1.4] ★纯同步 parseTagBankLorebook：ST 世界书 → 标签词库
  │                                    `[分类]：名字` 出分类与名字、`key[]` 出别名、`- 名字：tag, tag` 出标签
  │                                    🔴 **一条也不抛**（照 planInstall）：几千条手写语料里一定有没见过的
  │                                       写法，抛错等于让一个畸形条目否掉整本。读不懂的记进 notes 跳过
  │                                    🔴 `,` 与 `/` **语义不同**：逗号 = 同时成立，斜杠 = 同类候选。
  │                                       压平的话 AI 会把六种耳朵一起写进提示词，模型就给画六对
  │                                    🔴 冒号左边**含中日韩字符且够短**才当标签名剥掉 —— 只看冒号会把
  │                                       `rating:general` 与 NAI 的 `1.5::tag::` 拦腰截断，**且是静默的**
  │                                       （截出来的前半段仍是语法合法的标签串，只是意思全变了）
  │                                    🔴 BOM / 零宽 / U+FFFD 在导入期就清掉并留痕：编码闸门扫的是 data/
  │                                       与源码，**扫不到用户运行时导入的库**，这道关只能由导入器自己把
  ├── image-tag-bank.ts             ← [图像 v1.4] ★纯函数：目录渲染 + 两个查询工具的实现
  │                                    检索模型是**AI 看目录 → 调工具 → 拿标签 → 自己组装**（用户裁定）
  │                                    🔴 **目录必须逐字节稳定**：它是几万字符、每张图都发一遍的东西，
  │                                       唯一能让它不烧钱的办法是落进 prompt cache 的稳定前缀
  │                                       （同 LORE_BOOK_STATIC 的静态区）。故排序**不用 localeCompare**
  │                                       （依赖运行环境 ICU 数据）、不掺时钟、不掺随机
  │                                    🔴 目录里**只有名字**；`alwaysOn`（上游 constant）那一档是例外，
  │                                       连标签一起印 —— 为「每张图都成立」的条目各跑一次工具往返是浪费
  │                                    🔴 超上限时**照实说漏了多少条**并指路 search_image_tags：
  │                                       静默截断会让模型以为目录就是全部，它不会去搜自己不知道存在的东西
  │                                    🔴 `lookupTagEntries` 查不到就回报 notFound，**不模糊猜一条**——
  │                                       目录里的名字是模型刚读到的字符串，写不对就是真写错了，
  │                                       而模糊匹配会把「写错」变成「安静地给了另一条的标签」
