...
import type { ImageProviderId, QuotaReason, QuotaVerdict, SceneImageRecord } from './types-image';
...
export interface QuotaInput {
   ...
   * 传进来的每一条都算。
   *
   * 🔴 **`provider` 必须带上**（图像 v2 / C9）：L1/L2 是花钱防线，只有**付费后端**画过的
   * 那些才占预算。少了这一格，25 张免费的本地图会把 NovelAI 的每小时额度吃干净 ——
   * 切回 NAI 时第一张付费图就被报成「已达本小时上限（25/20）」，而付费的一张都还没画过。
   */
  records: readonly Pick<
    SceneImageRecord,
    'messageId' | 'turn' | 'source' | 'createdAt' | 'provider'
  >[];
  /** 本次要生成的目标 */
  ...
  /**
   * **既有记录**里那个 provider 由谁付钱（图像 v2 / C9）。
   *
   * 能力表 `PROVIDER_CAPABILITIES` 住在接线层（`scene-image-seams.ts`，全仓唯一一张），
   * 引擎不认识它 —— 所以这一格由调用方交进来，而不是在这里抄第二张表。
   * 入参是记录里那一格原样的值：**`undefined` 由调用方解释**（约定读作 `novelai`，
   * 见 `SceneImageRecord.provider`），本模块不替它决定。
   *
   * 🔴 缺省 = **一律按 `'paid'` 计**，也就是图像 v1 的行为。这与上面 `costModel` 的
   *    「必填无默认」不矛盾：那一格两个方向都错得无声（paid 当 local 是不该花的钱花了），
   *    而这一格的缺省只会**多算**几条记录 —— 多算的后果是自动档降级成手动按钮、
   *    手动档多一次确认，两条都不花钱。宁可多拦一张，不可漏掉一轮扣费。
   */
  costModelOf?: (provider: ImageProviderId | undefined) => 'paid' | 'local';
}
 ...
 * | 层 | 判据 | 计谁 | 计哪些记录 | 哪种后端 |
 * | --- | --- | --- | --- | --- |
 * | L1 每条消息 | 同 `messageId` 的记录数 ≥ `maxPerMessage` | auto + manual 都计 | **仅付费记录** | **仅 `paid`** |
 * | L2 滚动时间窗 | `now - createdAt < IMAGE_QUOTA_WINDOW_MS` 的记录数 ≥ `maxPerHour` | auto + manual 都计 | **仅付费记录** | **仅 `paid`** |
 * | L3 同回合去重 | 目标是 `auto` 且同 `turn` 已有 `auto` 记录 | **只对 auto 生效** | 全部记录 | **两种都开** |
 *
 * L3 只拦自动档：玩家想为同一段剧情多画几张，是他的钱、他的选择（§5.3 不变式）。
 * L1/L2 两种 source 都计：一个 UI bug 造成的连点也该被拦。
 *
 * 🔴 **「哪种后端」与「计哪些记录」是两件事**（C9，2026-08-08 评审补）：前者问的是「这一张
 *    要不要花钱」，后者问的是「预算已经被谁吃掉了」。只判前者的话，本地后端画的图会去啃
 *    付费后端的预算 —— 免费画 25 张之后切回 NAI，第一张付费图就被拦，而账单上一分钱都没有。
 *    L3 是**正确性**规则不是花钱规则，所以它两边都不筛（见下）。
 *
 * 多层同时不满足时，按上表顺序报**第一条** —— 裁决必须是确定的，否则 tooltip 会随记录顺序变脸。
 */
export function checkQuota(input: QuotaInput): QuotaVerdict {
  const { records, target, now, limits, costModel, costModelOf } = input;

  // 🔴 L1/L2 是**花钱防线**，`local` 后端整条跳过（C9）。本地画一张只花自己的显卡时间，
  //    给它设上限是把一条为付费上游写的规则套在没有账单的地方。
  const spendGuarded = costModel === 'paid';

  /** 这一条记录当初是不是花过钱。缺 `costModelOf` 时一律算花过（见字段注释：只会多拦） */
  const paidRecord = (provider: ImageProviderId | undefined): boolean =>
    (costModelOf?.(provider) ?? 'paid') === 'paid';

  // ── L1 每条消息 ──
  if (spendGuarded) {
    let perMessage = 0;
    for (const r of records) {
      if (r.messageId === target.messageId && paidRecord(r.provider)) perMessage += 1;
    }
  ...
  if (spendGuarded) {
    let inWindow = 0;
    for (const r of records) {
      if (now - r.createdAt < IMAGE_QUOTA_WINDOW_MS && paidRecord(r.provider)) inWindow += 1;
    }
  ...
  //
  // 🔴 同理，这里**不筛 `paidRecord`**：上一张是本地画的，这一回合照样已经有插画了。
  //    按付费与否筛会让「本地画过一张、切到 NAI 后自动档又画一张」变成常态。
