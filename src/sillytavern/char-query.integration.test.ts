...
describe('getNpcs saveId 集成（修 #30）', () => {
  ...
  it('不同存档的 NPC 互不干扰', async () => {
    await saveCharacter(createDefaultCharacterState({ id: 'a', name: 'A', type: 'npc', saveId: 's1' }));
    await saveCharacter(createDefaultCharacterState({ id: 'b', name: 'B', type: 'npc', saveId: 's2' }));
    await saveCharacter(createDefaultCharacterState({ id: 'c', name: 'C', type: 'npc', saveId: 's1' }));
  ...
  it('player 类型不受 getNpcs 影响', async () => {
    await saveCharacter(createDefaultCharacterState({ id: 'p1', type: 'player', saveId: 'save_Q' }));
    await saveCharacter(createDefaultCharacterState({ id: 'n1', type: 'npc', saveId: 'save_Q' }));
...
describe('getMonsters saveId 集成', () => {
  it('getMonsters(saveId) 只返回该存档的怪物', async () => {
    await saveCharacter(createDefaultCharacterState({ id: 'm1', type: 'monster', saveId: 'save_Q' }));
    await saveCharacter(createDefaultCharacterState({ id: 'm2', type: 'monster', saveId: 'save_Z' }));
