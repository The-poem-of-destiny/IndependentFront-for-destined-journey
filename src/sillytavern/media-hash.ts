 ...
 *
 * 🔴 **住在引擎侧**（原先在 `src/ui/lib/media-hash.ts`）: 消费方横跨两层 ——
 * 引擎的 `content-source.ts`（pack 分节 hash）与前端的 asset-zip / asset-store /
 * audio-store / scene-image-seams。放前端就只能让引擎反向 import 前端，
 * 那是分层闸门（`tests/layering-gate.test.ts`）明令禁止的方向；而本模块零 DOM 依赖
 * （`crypto.subtle` 惰性 + 特性检测），本来就该在引擎。`src/ui/lib/media-hash.ts`
 * 留一个转发壳，前端四处 import 路径一字未改。
 *
