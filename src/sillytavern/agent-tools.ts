...
import type { SettlementResult } from './combat-settlement-pipeline';
...
export const ALL_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  // ── Combat Control Tools (战斗控制类, M4 任务 5.3) ──
  // 底层走管道版 (combat-pipeline.ts / combat-actions-pipeline.ts / combat-settlement-pipeline.ts),
  // 需要 PipelineContext (EventBus + combatants + 战斗实例), 暂留占位等 orchestrator 接入。
  {
    type: 'function',
    function: {
      name: 'combat_start',
      description:
        '开战。传入战斗类型、敌我角色名列表（按名寻址，铁律1）、环境描述、各单位先攻 d20。这是真实计算（先攻排序代码做），不是猜测。必须先调 roll_d20 拿到每个单位的先攻 d20，禁止编造骰值。',
      parameters: {
        type: 'object',
        properties: {
          combatType: {
            type: 'string',
            enum: ['切磋', '竞技', '压制', '死斗', '标准', '守卫'],
            description: '战斗类型（决定战意阈值/EXP 系数）',
          },
          allies: {
            type: 'array',
            items: { type: 'string' },
            description: '我方角色名列表（按名寻址）',
          },
          enemies: {
            type: 'array',
            items: { type: 'string' },
          ...
          environment: { type: 'string', description: '战斗环境描述' },
          d20Rolls: {
            type: 'array',
            items: { type: 'integer' },
            description: '各单位先攻 d20，顺序 = allies + enemies',
          },
        },
        required: ['combatType', 'allies', 'enemies', 'environment', 'd20Rolls'],
      },
    },
  },
  {
    ...
    function: {
      name: 'combat_attack',
      description:
        '执行单次攻击（核心）。走 19 event 攻击链：意图解析→检定→8步伤害管线→HP红线→战意。HP扣减/生死判定由代码结算，AI 只读结果做叙事。这是真实计算，不是猜测。涉及骰值的必须先调 roll_d20 拿到 d20Attack，禁止编造骰值。',
      parameters: {
        type: 'object',
        properties: {
          attackerId: { type: 'string', description: '攻击者角色名' },
          defenderId: { type: 'string', description: '目标角色名' },
          intentionInput: {
            type: 'string',
            description: '用户原始输入（意图解析用，如"砍向要害"），缺省=常规',
          },
          damageType: {
            type: 'string',
            enum: ['物理', '能量', '精神', '真实'],
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '使用技能（非攻击型）。触发 combat.action.use 事件。技能效果实际由技能定义驱动。这是真实流程，不是猜测。',
      parameters: {
        type: 'object',
        properties: {
          characterId: { type: 'string', description: '施法者角色名' },
          skillName: { type: 'string', description: '技能名' },
        },
        required: ['characterId', 'skillName'],
      },
    },
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          characterId: { type: 'string', description: '使用者角色名' },
          itemName: { type: 'string', description: '道具名' },
        },
        required: ['characterId', 'itemName'],
      },
    },
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          characterId: { type: 'string', description: '角色名' },
        },
        required: ['characterId'],
      },
    },
  },
  {
    ...
    function: {
      name: 'combat_move',
      description:
        '战术移动。节点式 location 无坐标，位置变更归 Story 叙事，本动作仅触发 combat.action.use 事件。这是真实流程，不是猜测。',
      parameters: {
        type: 'object',
        properties: {
          characterId: { type: 'string', description: '角色名' },
  ...
  },
  {
    ...
    function: {
      name: 'combat_flee',
      description:
        '逃跑检定。敏捷+d20 vs DC(15+敌方平均层级×2)。成功仅结算不位移。这是真实计算，不是猜测。必须先调 roll_d20 拿到 d20Roll，禁止编造骰值。',
      parameters: {
        type: 'object',
        properties: {
          characterId: { type: 'string', description: '角色名' },
          d20Roll: {
            type: 'integer',
            description: '逃跑检定 d20（必须先调 roll_d20 取真实值）',
          },
        },
        required: ['characterId', 'd20Roll'],
      },
    },
  },
  {
    ...
    function: {
      name: 'combat_end',
      description:
        '结束战斗 + 结算。走 runSettlementPipeline：combat.end → EXP → settle.loot → settle.complete。EXP 公式代码算（Lv×战斗系数×集群衰减），FP/摘要 AI 写。这是真实计算，不是猜测。',
      parameters: {
        type: 'object',
        properties: {
          winner: {
            type: 'string',
            enum: ['ally', 'enemy', 'draw'],
            description: '胜方',
          },
        },
        required: ['winner'],
  ...
  // ── Status Tools (状态类, M4 任务 5.3) ──
  // 底层走 status-api.ts applyStatusIntents/removeStatusIntents + buff-registry,
  // 战斗内施加 buff 需配合 PipelineContext 落库时机, 暂留占位等 orchestrator 接入。
  {
    type: 'function',
    function: {
      name: 'status_apply',
      description:
        '施加 buff（自动走 id 去重：同源刷新+增层/异源独立）。buff id = sourceKey.name（环境 buff 才允许裸名）。去重代码做，AI 只声明 buff 描述。这是真实流程，不是猜测。',
      parameters: {
        type: 'object',
        properties: {
          target: { type: 'string', description: '目标角色名' },
          name: { type: 'string', description: 'buff 名（如"流血"）' },
          category: {
            type: 'string',
            enum: ['增益', '减益', '特殊'],
            description: 'buff 分类',
          ...
          sourceKey: {
            type: 'string',
            description: '来源前缀（物品/技能名，buff id=sourceKey.name；环境 buff 才允许裸名）',
          },
          stacks: { type: 'integer', description: '层数，缺省 1' },
          duration: { type: 'integer', description: '持续回合（战斗型有效）' },
          lifecycle: {
            type: 'string',
            enum: ['战斗', '持续', '触发', '条件'],
            description: '生命周期',
          ...
          divinity: {
            ...
            minimum: 0,
            maximum: 8,
            description: '登神等级 0-8（神位级 buff 才填）',
          },
          effects: {
            type: 'object',
  ...
  },
  {
    ...
    function: {
      name: 'status_remove',
      description:
        '移除 buff。传完整 buffId（如"剑.流血"）或裸 name（裸 name 移除所有同名）。这是真实流程，不是猜测。',
      parameters: {
        type: 'object',
        properties: {
          target: { type: 'string', description: '目标角色名' },
          buffIdOrName: {
            type: 'string',
            description: '完整 buffId（"剑.流血"）或裸 name',
          },
        },
        required: ['target', 'buffIdOrName'],
      },
    },
  },
  {
    ...
    function: {
      name: 'status_query',
      description:
        '查询角色 buff。不填 buffIdOrName 返回全部 StatusEffect；填了返回该 buff 的 {has, stacks}。只读查询，不改状态。',
      parameters: {
        type: 'object',
        properties: {
          target: { type: 'string', description: '角色名' },
          buffIdOrName: {
            type: 'string',
            description: '查指定 buff（可选，缺省返回全部）',
          },
        },
        required: ['target'],
      },
...
/**
 * combat 工具独立执行通道的上下文（B 方案，不走 ToolExecutionContext）。
 *
 * 现有 ToolExecutionContext 只有 { characters, variables, saveId }，缺 bus/combatants/战斗实例；
 * combat 工具需要 EventBus（19 event emitChain）+ 在场过滤 + 当前 CombatState，
 * 故新建独立通道 executeCombatToolCall + CombatToolContext，保持 craft/char/item 工具兼容。
 *
 * 设计: 纯分发 —— 调底层管道函数 + 把返回值包进 ToolCallResult.result；
 *       不直接改 ctx.combat、不落库，patches 收集在 result 里交给调用方（combat_runner）。
 */
export interface CombatToolContext {
  characters: CharacterState[];
  variables: Record<string, any>;
  saveId: string;
  /** 19 event emitChain 用 */
  bus: EventBus;
  /** 在场过滤（参战者 charId 列表） */
  combatants: string[];
  /** 当前战斗实例（combat_start 后由调用方 combat_runner 维护更新） */
  combat: CombatState;
  /** 只读查询钩子（M1，供 handler / $status 查角色状态） */
  readHooks?: ReadonlyHookSet;
}

/** 工具执行结果（与 types.ts 一致，这里重新声明以避免循环依赖歧义；combat 通道统一返回此形状） */
export interface CombatToolCallResult {
  toolCallId: string;
  functionName: string;
  result: any;
  error?: string;
...
// ═══════════════════════════════════════════════════════════
// Group E: Combat 工具独立执行通道 (B 方案, M4 任务 5.2)
//
// combat agent 走独立通道，不污染现有 ToolExecutionContext。
// 纯分发: 调底层管道函数 + 包进 CombatToolCallResult.result;
// 不直接改 ctx.combat、不落库，patches 收集在 result 里交给调用方 (combat_runner)。
// 对齐: docs/reference/combat-agent-api.md §2/§4 + §8
// ═══════════════════════════════════════════════════════════

/**
 * 从 CombatToolContext 派生 PipelineContext（复用，保证 combatants/readHooks/combatType 一致）。
 * currentRound 取 ctx.combat.round。
 */
function buildPipelineCtx(ctx: CombatToolContext): PipelineContext {
  return {
    bus: ctx.bus,
    combatants: ctx.combatants,
    readHooks: ctx.readHooks,
    currentRound: ctx.combat.round,
    combatType: ctx.combat.combatType,
  };
}

/**
 * 按名寻址（铁律1：AI 永不产 id，combat 工具收到的是角色名）。
 * 从 ctx.combat.participants 按 name 找 characterId。
...
/**
 * combat 工具执行器（独立通道，B 方案）。
 *
 * 分发 12 个 combat/status 工具到底层管道函数（纯分发，不落库）:
 *  - combat_start     → initCombat（角色名→CombatParticipant 转换）
 *  - combat_attack    → resolveAttackPipeline（19 event 攻击链）
 *  - combat_use_skill → resolveUseSkill
 *  - combat_use_item  → resolveUseItem
 *  - combat_block     → resolveBlock
 *  - combat_move      → resolveMove
 *  - combat_focus     → resolveFocus
 *  - combat_flee      → resolveFlee
 *  - combat_end       → runSettlementPipeline（EXP/FP/战利品/摘要）
 *  - status_apply     → applyStatusIntents（buff 去重 → patches）
 *  - status_remove    → removeStatusIntents
 *  - get_combat_state → getCombatState（只读快照）
 *
 * 调用方 (combat_runner) 注意:
 *  - combat_start 返回新 CombatState，**必须**用它更新 ctx.combat（后续工具依赖）
 *  - combat_attack 的 patches（delta_hp 等）由调用方统一走 state-manager.commitChatState 落库
 *  - status_apply/remove 的 patches 同上，由调用方落库
 *  - combat_end 返回 SettlementResult（含 exp/fp?/patches/summary），patches 由调用方落库
 *
 * @param name      工具名（如 'combat_start'）
 * @param args      AI 传入的参数对象
 * @param ctx       combat 独立上下文
 * @param toolCallId 可选，工具调用 ID（透传进结果）
 */
export async function executeCombatToolCall(
  name: string,
  args: Record<string, any>,
  ctx: CombatToolContext,
  toolCallId = `combat_call_${Date.now()}`,
): Promise<CombatToolCallResult> {
  try {
    const result = await dispatchCombatTool(name, args, ctx);
    return { toolCallId, functionName: name, result };
  } catch (err: any) {
    return {
      toolCallId,
      functionName: name,
...
/** combat 工具纯分发（可能抛错，由 executeCombatToolCall 包成 error） */
async function dispatchCombatTool(
  name: string,
  args: Record<string, any>,
  ctx: CombatToolContext,
): Promise<any> {
  switch (name) {
    // ── 战斗控制 ──
    case 'combat_start': {
      const { initCombat, characterToCombatParticipant } = await import('./combat-resolver');
      const alliesNames: string[] = args.allies ?? [];
      const enemiesNames: string[] = args.enemies ?? [];
      ...
      const combat = initCombat({
        ...
        d20Rolls: args.d20Rolls ?? [],
      });
      // 返回完整 CombatState 摘要给 AI（调用方据此更新 ctx.combat）
      return {
        ...
        // 原始 CombatState 供调用方更新 ctx.combat
        _combatState: combat,
      };
    }

    case 'combat_attack': {
      const { resolveAttackPipeline } = await import('./combat-pipeline');
      const attackerId = findCharIdByName(ctx, args.attackerId);
      const defenderId = findCharIdByName(ctx, args.defenderId);

      const input: AttackInput = {
        combat: ctx.combat,
        attackerId,
        defenderId,
        userInput: args.intentionInput,
        action: 'attack',
      ...
      // CombatActionResult 完整返回（含 patches/damage/finalHp/isDead，调用方落库 patches）
      return result;
    }

    case 'combat_use_skill': {
      const { resolveUseSkill } = await import('./combat-actions-pipeline');
      const characterId = findCharIdByName(ctx, args.characterId);
      const pipelineCtx = buildPipelineCtx(ctx);
      return await resolveUseSkill(characterId, args.skillName, pipelineCtx);
    }

    case 'combat_use_item': {
      const { resolveUseItem } = await import('./combat-actions-pipeline');
      const characterId = findCharIdByName(ctx, args.characterId);
      const pipelineCtx = buildPipelineCtx(ctx);
      return await resolveUseItem(characterId, args.itemName, pipelineCtx);
    }

    case 'combat_block': {
      const { resolveBlock } = await import('./combat-actions-pipeline');
      const characterId = findCharIdByName(ctx, args.characterId);
      const pipelineCtx = buildPipelineCtx(ctx);
      return await resolveBlock(characterId, pipelineCtx);
    }

    case 'combat_move': {
      const { resolveMove } = await import('./combat-actions-pipeline');
      const characterId = findCharIdByName(ctx, args.characterId);
      const pipelineCtx = buildPipelineCtx(ctx);
      return await resolveMove(characterId, pipelineCtx);
    }

    case 'combat_focus': {
      const { resolveFocus } = await import('./combat-actions-pipeline');
      const characterId = findCharIdByName(ctx, args.characterId);
      const pipelineCtx = buildPipelineCtx(ctx);
      return await resolveFocus(characterId, pipelineCtx);
    }

    case 'combat_flee': {
      const { resolveFlee } = await import('./combat-resolver');
      const characterId = findCharIdByName(ctx, args.characterId);
      // resolveFlee 是同步纯函数（签名 combat/characterId/d20Roll）
      return resolveFlee(ctx.combat, characterId, args.d20Roll);
    }

    case 'combat_end': {
      const { runSettlementPipeline } = await import('./combat-settlement-pipeline');
      const pipelineCtx = buildPipelineCtx(ctx);
      const result: SettlementResult = await runSettlementPipeline(
        ctx.combat,
        args.winner,
        pipelineCtx,
      );
    ...
    // ── 状态类 ──
    case 'status_apply': {
      const { applyStatusIntents } = await import('./status-api');
      const target = args.target;
      // target 是角色名，patch target 需用 characterId（status-api 用 target 字段拼 characters.${target}）
      // 按名查 characterId，传给 StatusApplyIntent.target
      const targetId = findCharIdByName(ctx, target);
      const existing = findStatusEffectsByName(ctx, target);

      // 构造 buffDef（对齐 StatusApplyIntent: name + category 必，其余可选）
      const buffDef: StatusApplyIntent['buffDef'] = {
        name: args.name,
        ...
        description: args.description ?? `${args.sourceKey ?? '环境'}.${args.name}`,
      };

      const intent: StatusApplyIntent = { target: targetId, buffDef };
      const r = applyStatusIntents(existing, [intent]);
      return {
        action: r.results[0]?.action ?? 'added',
        buffId: r.results[0]?.buffId,
        patches: r.patches,
        updated: r.updated,
      };
    }
    ...
    case 'status_remove': {
      const { removeStatusIntents } = await import('./status-api');
      const target = args.target;
      const targetId = findCharIdByName(ctx, target);
      const existing = findStatusEffectsByName(ctx, target);

      const intent: StatusRemoveIntent = {
        target: targetId,
        buffIdOrName: args.buffIdOrName,
      };
      const r = removeStatusIntents(existing, [intent]);
      return {
        removed: args.buffIdOrName,
        patches: r.patches,
        updated: r.updated,
      };
    }
    ...
    // ── 只读查询 ──
    case 'get_combat_state': {
      const { getCombatState } = await import('./combat-resolver');
      return {
        summary: getCombatState(ctx.combat),
        combatId: ctx.combat.combatId,
