...
export function buildOverviewPanel(state: CombatState): string {
  ...
  for (const p of state.participants) {
    ...
    lines.push(
      `  | [${sideLabel}] ${p.name}${clusterInfo}: HP ${p.hp}/${p.maxHp} | MP ${p.mp}/${p.maxMp} | SP ${p.sp}/${p.maxSp} | 力${p.attributes.str} 敏${p.attributes.dex} 体${p.attributes.con} 智${p.attributes.int} 精${p.attributes.spi} |`,
    );

    if (p.statusEffects.length > 0) {
      const statusStr = p.statusEffects.map((s) => `${s.name}(${s.remainingTime}回合)`).join(', ');
...
export function buildInitiativePanel(order: CombatUnitTurn[]): string {
  ...
  for (const u of order) {
    ...
    lines.push(
      `  | ${u.name}: (敏捷${u.agility}${speedModStr}) + 骰${u.d20Roll}${fixedStr} = ${u.totalInitiative} |`,
    );
  }
...
export function buildAttackPanel(params: {
  ...
  lines.push(
    `  | 攻方: ${params.attackerName} | 守方: ${params.defenderName} | 招式: ${params.skillName} | 所使武器: ${params.weaponName} |`,
  );
  ...
  if (params.damage.multiSplitInfo) {
    ...
    lines.push(
      `  | 多段/连击(${ms.count}): ${params.damage.initialDamage} / ${ms.count} = ${ms.perHit} |`,
    );
  }
  ...
  if (pen.penetrationRate > 0) {
    lines.push(
      `  | 穿透修正: 防御${pen.originalDef} × (1-穿透${pen.penetrationRate * 100}%) = 有效防御${pen.effectiveDef} |`,
    );
  }

  const dmgType = params.request.damageType || '物理';
  lines.push(
    `  | ${dmgType}伤害: 构成${params.damage.afterMultiSplit} - 装备减免${params.damage.equipmentReduction} - 属性减免${params.damage.typeReductionAmount} = 结算基值${params.damage.afterTypeReduction} |`,
  );
  ...
  if (params.intention.contested) {
    ...
    lines.push(
      `  | 意图判定: ${c.attackerFormula} = ${c.attackerValue} vs ${c.defenderFormula} = ${c.defenderValue} → ${params.intention.verdict} |`,
    );
  } else {
  ...
  lines.push(
    `  | 伤害修正: (基础${params.damage.afterTypeReduction} × 评级系数${params.damage.ratingCoefficient} × 意图系数${params.damage.intentionCoefficient}) = ${afterRating} |`,
  );

  if (params.damage.drRate > 0) {
    lines.push(
      `  | DR修正: ${afterRating} × (1-DR${params.damage.drRate * 100}%) = ${params.damage.afterDr} |`,
    );
  }
...
export function buildFullActionPanel(params: {
  ...
  lines.push(`  | 消耗: HP ${costs.hp ?? 0} | MP ${costs.mp ?? 0} | SP ${costs.sp ?? 0} |`);
  lines.push(
    `  | 意图: ${params.request.intentionKeywords || '常规'} → 判定为 ${params.intention.level} |`,
  );
  ...
  if (pen.penetrationRate > 0) {
    lines.push(
      `  | 穿透修正: 防御${pen.originalDef} × (1-穿透${(pen.penetrationRate * 100).toFixed(0)}%) = 有效防御${pen.effectiveDef} |`,
    );
  }
  ...
  const afterRating = params.damage.afterRating;
  lines.push(
    `  | 伤害修正: (基础${params.damage.afterTypeReduction} × 评级系数${params.damage.ratingCoefficient} × 意图系数${params.damage.intentionCoefficient}) + 额外固定伤害0 = ${afterRating} |`,
  );

  // 效果修正 (如果有减伤类型)
  if (params.damage.typeReductionRate > 0) {
    lines.push(
      `  | 效果修正: ${afterRating} × ${dmgType}修正${(params.damage.typeReductionRate * 100).toFixed(1)}% × 机制修正1.0 = ${params.damage.afterDr} |`,
    );
  }
  ...
  if (params.damage.drRate > 0) {
    lines.push(
      `  | DR修正: ${params.damage.afterDr} × (1-DR${(params.damage.drRate * 100).toFixed(0)}%) = ${damageAfter(params.damage)} |`,
    );
  }

  lines.push(
    `  | 最终伤害: ${params.damage.finalDamage} | 变更: ${params.defenderName} HP ${params.defenderHp.before} → ${params.defenderHp.after} (${hpPercent(params.defenderHp.after, params.defenderHp.max)}%) (-${params.damage.finalDamage}) |`,
  );
  lines.push(
    `  | 是否存活: ${params.defenderHp.after} > 0 → [${params.isDead ? '死亡/毁坏' : '未死/存活'}] |${params.nonLethalNote ? ` (${params.nonLethalNote})` : ''}`,
  );
  ...
  for (const s of params.statusApplied) {
    ...
    if (s.triggered) {
      lines.push(
        `  | 状态效果: ${params.defenderName}获得${s.name}(${s.effect}), 持续${s.duration}回合 |`,
      );
    }
  }
